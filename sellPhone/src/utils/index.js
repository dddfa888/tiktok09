...
// 进入客服中心
export const openService = (flag = false, type = 'one') => {
  const mode = import.meta.env.MODE;
  const systemStore = useSystemStore();

  // 拆分接口返回的客服链接
  const rawUrl = systemStore?.customer_service_url || '';
  const urlList = rawUrl ? rawUrl.split(",") : [];

  console.log('urlList', urlList);

  // 根据 type 取不同客服链接
  let customerServiceUrl = "";
  if (type === "one") {
    window.open(urlList[0] || "");
    customerServiceUrl= urlList[0] || "";
  } else if (type === "two") {
    window.open(urlList[1] || "");
    customerServiceUrl= urlList[1] || "";
  } else {
    // 默认取第一个
     window.open(urlList[0] || "");
     customerServiceUrl= urlList[0] || "";
  ...
  const path = customerServiceUrl ? '' : '/customerService'
  ...
  if (customerServiceUrl) {
    if (window.plus) {
      window.plus.runtime.openURL(customerServiceUrl);
    } else if (window.webkit) {
      window.webkit.messageHandlers.openWindow.postMessage({ url: customerServiceUrl });
    } else {
  ...
  if (isMobile()) {
    href = hostname === 'localhost'
      ? 'https://tiktokmall666.com/www/#/customerService'
      : `${origin}/www/#/customerService`;
  } else {
    href = hostname === 'localhost'
      ? 'https://tiktokmall666.com/ww/#/login'
      : `${origin}/ww/#/login`;
  }
...
//       let href = ''
//       if (isMobile()) {
//         href = hostname === 'localhost' ? 'https://tiktokmall666.com/www/#/customerService' : `${origin}/www/#/customerService`
//       } else {
//         href = hostname === 'localhost' ? 'https://tiktokmall666.com/ww/#/login' : `${origin}/ww/#/login`
//       }
...
const images = import.meta.glob('@/assets/imgs/footer/**/*.png', { eager: true });

export const getImg = (filePath) => {
  console.log("----",filePath)
  const fullPath = `/src/assets/${filePath}`;
  const match = Object.entries(images).find(([key]) => key.includes(fullPath));
