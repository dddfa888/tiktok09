  ...
  <div class="shop-header">
    <div class="shop-header-content">
      ...
      <div class="enter" :class="{ 'is-ar': isArLang }">
        ...
        <div class="icon-wrapper">
          <img :src="images.horn.href" alt @click="openPage('/notice')" />
          <div v-if="noticeCount" class="count">{{ noticeCount > 99 ? 99 : noticeCount }}</div>
        </div>
      </div>
    </div>
    <div v-if="setStep" class="step-content">
      <div class="step-content-item">
        <div
          v-for="(item, index) in verifyStepDataRef"
          :key="index"
          :class="{ active: index < setStep - 1 }"
          class="item"
        >
          <div class="icon"></div>
          <p>{{ t(item.name) }}</p>
    ...
    <div v-if="show" class="mask" @click.self="close">
      <div class="box">
        ...
        <div class="item" @click="select('one')">{{ $t("onLineService") }}1</div>
        <div class="item" @click="select('two')">{{ $t("onLineService") }}2</div>

        <button class="close-btn" @click="close">{{ $t("cancel") }}</button>
      </div>
    </div>
  </div>
</template>
...
const images = {
  ...
  horn: new URL('@//assets/image/shop/Horn.png', import.meta.url)
}
...
const select = (type) => {
  if (type === 'one') {
    openService(false, 'metashop')
  } else {
    openService(false, 'metashop2')
  }
  close()
}
...
const getCount = async (flag) => {
  ...
  const pollUnreadNotice = () => {
    if (noticeInterval.value) {
      clearInterval(noticeInterval.value)
    }
    noticeInterval.value = setInterval(async () => {
      const res = await _getUnreadNotice({})
