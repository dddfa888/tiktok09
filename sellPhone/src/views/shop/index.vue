  ...
  <div :class="{ 'not-scroll': showNoticPop }" class="page-main-content">
    ...
    >
      ...
      <div :class="{ active: currentActivityId }" class="activity-pop-content">
        <div class="content">
          <img
            :src="getImg('activity/turntable/choujiangclose.svg')"
            class="close"
            @click="currentActivityId = null"
            alt=""
          />
          <img
            :src="getImg('activity/turntable/choujiang.png')"
            alt=""
            @click="openActivity"
          />
      ...
      <div class="shop-main-content">
        <div style="padding: 0 15px">
              ...
              >
                <div class="number">
                  <count-to
                    ...
                    class="number-count"
                  ></count-to>
                  <h3 v-else class="number-count">
                    {{ item.decimals ? '0.00' : 0 }}
                  </h3>
                </div>
                <p>{{ t(item.title) }}</p>
          ...
          <div class="stat-item-content">
            <div
              v-for="(item, index) in statItemDataRef"
              :key="item.title"
              class="item"
            >
              <div class="icon"><img :src="item.icon" alt="" /></div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const openActivity = () => {
      ...
      if (window.plus) {
        ...
        window.plus.runtime.openURL(
          `${origin}/www/#/activity/turntable?id=${currentActivityId.value}&token=${token}`
        )
      } else {
        openPage(
          `${origin}/www/#/activity/turntable?id=${currentActivityId.value}`,
          true
        )
      }
    }
    ...
    if (['familyShop'].includes(mode)) {
      const index = navDataRef.findIndex(
        (item) => item.href === '/shop/promotion'
      )
      navDataRef.splice(index, 1)
    }
    ...
    const pageDataInit = async (flag = false) => {
      ...
      if (sellerId.value) {
        ...
        sellerReportHead({ sellerId: sellerId.value }).then((res) => {
          console.log(res.head)
          statBlockDataRef.value[0].number = res.head.orderNum || 0
          statBlockDataRef.value[2].number = res.head.unconfirmed || 0
          statBlockDataRef.value[1].number = res.head.orderCancel || 0
          statBlockDataRef.value[3].number = res.head.sendOutOrder || 0
          statBlockDataRef.value[4].number = res.head.payOrder || 0
          statBlockDataRef.value[5].number = res.head.arrivedGoods || 0

          // statBlockDataRef.value[5].number = res.head.payOrder || 0
          // statBlockDataRef.value[4] &&
          //   (statBlockDataRef.value[4].number = res.head.orderRefund || 0)
        })
        ...
        // }).then((res) => {
        //   const data = res.head
        //   statBlockDataRef.value.forEach((item) => {
        //     item.number = data[item.key]
        //   })
        //   console.log(res.head, 'value')
        // })
    ...
    const openNavPage = (href) => {
      if (href === '/gotoBuyer') {
        ...
        const href =
          hostname === 'localhost' ? 'https://tiktokmall666.com' : `${origin}`
