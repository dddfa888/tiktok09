<template>
  <div class="goods_list" :class="{ 'is-ar': isArLang }">
    <div class="checkbox-container">
      <p class="title mb-2">{{ props.info.id }}</p>
      <van-checkbox
        v-model="checked"
        v-if="
          Number(props.info.purchStatus) !== 1 &&
          ![-1, 0, 6].includes(Number(props.info.status))
        "
        @change="onChange(props.info.id, $event)"
      >
      </van-checkbox>
      <!-- <input
    v-if="Number(props.info.purchStatus) !== 1 && ![-1, 0, 6].includes(Number(props.info.status))"
      type="checkbox"
      :checked="isChecked(props.info.id)"
      @change="onChange(props.info.id, $event)"
    > -->
    </div>
    <div @click="goDetails">
      <p class="mb-2">
        ...
        <span
          >{{ $t('下单日期') }}：{{
            formatZoneDate(props.info.createTime)
          }}</span
        >
      </p>
      <p class="mb-2">
        ...
        <span
          ...
          <span
            :style="{
              color: props.info.payStatus == 1 ? '#2369f6' : '#dc2626'
            }"
            >{{
              props.info.payStatus == 1 ? $t('买家已付款') : $t('等待买家付款')
            }}</span
          >
      ...
      <p class="mb-2">
        ...
        <span
          >{{ $t('采购状态') }}：{{
            props.info.purchStatus == 1 ? `${$t('已采购')}` : $t('待采购')
          }}</span
        >
      </p>
      <p class="mb-2">
        ...
        <span
          >{{ $t('物流状态') }}：{{ handleLogistics(props.info.status) }}</span
        >
      </p>
      <div class="bottom-content">
        <div class="money">
          <p>${{ numberStrFormat(props.info.prizeReal) }}</p>
          <span
            >({{ $t('利润') }}${{ numberStrFormat(props.info.profit) }})</span
          >
        </div>
...
const props = defineProps({
  ...
  orderItemIdList: Function
})
const checked = ref(false)
const onChange = (id, event) => {
  props.orderItemIdList(id, event)
}
const isTrueChecked = (e) => {
  checked.value = e
}
