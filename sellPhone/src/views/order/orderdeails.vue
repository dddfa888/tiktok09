  ...
  <div>
    ...
    <div class="section" :class="{ 'is-ar': isArLang }">
      ...
      <div
        ...
        style="cursor: pointer; color: #ccc; t-weight: bold"
      >
        {{ showMore ? ' ▲' : ' ▼' }}
      </div>
      <div v-if="showMore">
        <div class="info-list">
          <p>{{ $t('姓名') }}</p>
          <div>
            {{ orderInfo.goodType == 'virtual' ? '---' : orderInfo.username }}
          </div>
        </div>
        <div class="info-list">
          <p>{{ $t('邮箱') }}</p>
        ...
        <div class="info-list">
          <p>{{ $t('国家') }}</p>
          <div>
            {{ orderInfo.goodType == 'virtual' ? '---' : orderInfo.country }}
          </div>
        </div>
        <div class="info-list">
          <p>{{ $t('州') }}</p>
          <div>
            {{ orderInfo.goodType == 'virtual' ? '---' : orderInfo.province }}
          </div>
        </div>
        <div class="info-list">
          <p>{{ $t('城市') }}</p>
          <div>
            {{ orderInfo.goodType == 'virtual' ? '---' : orderInfo.city }}
          </div>
        </div>
        <div class="info-list">
          <p>{{ $t('邮编') }}</p>
    ...
    <div class="section bottom" :class="{ 'is-ar': isArLang }">
      <div class="list">
        {{ $t('小计') }}
        <span>${{ numberStrFormat(orderInfo.systemPrice) }}</span>
      ...
      <div v-if="orderInfo.sellerDiscountPrice" class="list price-content">
        ...
        <p>
          {{ t('采购优惠')
          }}<span>{{ new Decimal(orderInfo.sellerDiscount).mul(100) }}%</span>,
          {{ t('优惠价')
          }}<span>${{ numberStrFormat(orderInfo.sellerDiscountPrice) }}</span>
        </p>
      </div>
      <div class="list">
    ...
    <!-- <div
      class="right-fixed-enter"
      :class="{ active: showIconEnter }"
      @click="toLogistics"
    >
      <img :src="images.car" alt="" />
    </div>
    <div
      class="right-fixed-enter message"
      :class="{ active: showIconEnter }"
      @click="toClient"
    >
...
const getListData = () => {
  ...
  _orderDetailList(params)
    .then((res) => {
      ...
      shopList.value =
        page.value.pageNum === 1 ? data : [...shopList.value, ...data]

      shopList.value.forEach((item) => {
        if (item.goodType == '虚拟商品') {
          orderInfo.value.goodType = 'virtual'
        }
      })
