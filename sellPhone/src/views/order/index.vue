<template>
  <div id="order" :class="{'is-ar': isArLang}">
    ...
    <div class="main">
      ...
      <div class="btn-style">
        <van-button @click="openInputDialog()" type="primary">{{ $t('批量采购') }}</van-button>
        <van-button @click="allChoice()" type="default">{{ $t('全选') }}</van-button>
      </div>
      ...
      <van-popup v-model:show="showCenter2" round>
        <div @click="handleChoose2(item)"
          class="text-xs w-72 h-12 flex justify-center items-center border-bottom relative" :key="index + 'option2'"
      ...
      <div class="main-list">
        <van-pull-refresh v-if="list.length > 0" v-model="refreshing" @refresh="onRefresh" :loading-text="$t('加载中')"
          :loosing-text="$t('释放以刷新')" :pulling-text="$t('下拉以刷新')">
          <van-list :immediate-check="false" v-model:loading="loading" :loading-text="$t('加载中')" :finished="finished"
            :finished-text="$t('noMore')" @load="getOrderList">
            <div class="goods">
              <orderItem ref="changeCheckboxRef" :orderItemIdList="orderItemIdList" v-for="item in list" :info="item" />
    ...
    <van-action-sheet v-model:show="show" :title="t('请输入交易密码')">
      <div style="height: 22rem">
        <van-password-input :length="6" :value="value" :focused="showKeyboard" @focus="showKeyboard = true" />
...
<script>
  ...
  import { _buyOrder } from "@/service/order.api.js";
  ...
  export default {
    ...
    computed: {
      ...
      hasSetSafePass() {
        return this.userStore.userInfo && this.userStore.userInfo.safeword
      }
    },
    // 密码输入到6位发送请求
    watch: {
    ...
    methods: {
      ...
      allChoice() {
        ...
        this.$refs.changeCheckboxRef.forEach((item)=>{
          item.isTrueChecked(this.isAllchoice)
          if(!this.isAllchoice) {
            this.checkedItems = []
          }
        })
      ...
      purchasebatch() {
        const idList = this.checkedItems.join(',')
        const data = {
          orderId: idList,
          safeword: this.value,
        }
      ...
      openInputDialog() {
        ...
        this.purchasebatch()
        // if (this.hasSetSafePass) {
        //   this.show = true
        //   this.showKeyboard = true
        // } else {
        //   Toast(this.t('请设置资金密码'))
        //   setTimeout(() => {
        //     router.push({
