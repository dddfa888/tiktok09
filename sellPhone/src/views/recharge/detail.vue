<template>
  <div>
    ...
    <div v-if="currentType && currentType !== 'bank'" class="rechargeDetail">
      <ExRadioGroup :list="list" :label="t('blockchainNetwork')" v-model="channel" />

      <img v-if="qrCodeImgSrc" class="qrCode" :src="qrCodeImgSrc" alt />

      <div class="download">
        <van-button type="default" @click="handleDownload">{{ t('saveQrCode') }}</van-button>
      </div>
      ...
      <div class="my-uploader">
        ...
        <!--<van-uploader
          ...
          :deletable="!isUploaderDisabled"
          :disabled="isUploaderDisabled"
          :max-size="10000 * 1024"
          @oversize="onOversize"
        />-->
        <Uploader v-model:uploadImg="uploadImg" />
      </div>
    ...
    <div v-if="currentType === 'bank'" class="rechargeDetail bank-input-content">
      ...
      <div v-if="typeValue && currentTypeObj && !isGcash" class="recharge-limit">
        {{ t('充值限额') }}：
        <span>{{ currentTypeObj.min_amount }}</span> ~
        <span>{{ currentTypeObj.max_amount }}</span>
      </div>
      <div v-if="isGcash" class="recharge-limit">
        {{ t('充值限额') }}：
        <span>{{ gMin }}</span> ~
        <span>{{ gMax }}</span>
      </div>
    ...
    <div class="pay-button" v-if="payUrl">
      <div class="pay-button-content">
        <span class="pay-button-title">
          {{
          t('请前往{_$1}继续完成支付', { _$1: route.query.key })
          }}
        </span>
...
const pickImage = async () => {
  try {
    ...
    // 你可以把 image.webPath 用作 <img> src 或上传
  } catch (e) {
    console.error('获取图片失败', e)
  }
}
const isUploaderDisabled = computed(() => {
  return loading.value // 或者根据上传状态禁用
})

const handleClickUpload = () => {
  // 自定义上传点击逻辑，如调用原生插件等
  console.log('点击上传按钮')
  // 示例：直接让系统打开文件选择器，这里走默认逻辑
}
...
const getSessionToken = () => {
  return new Promise((resolve, reject) => {
    exchangeSetRechargeToken({ session_token: userStore?.userInfo?.token })
      .then((res) => {
...
const typeColumns = computed(() => {
  return bankData.value.map((item) => ({
    text: item.bank_code,
...
const bankRechargeHandle = () => {
  ...
  if (!isNaN(amountNum)) {
    const min_amount = isGcash.value
      ? gMin.value
      : currentTypeObj.value.min_amount
    const max_amount = isGcash.value
      ? gMax.value
      : currentTypeObj.value.max_amount
    ...
    getSessionToken().then(async () => {
      ...
      await ajaxFn(params, rechargeType)
        .then((res) => {
          amount.value = ''
          loading.value = false
          payUrl.value = res
