...
if ('partyId' in route.query) {
  dataObj = {
    ...
    lang: localStorage.getItem('lang') === 'vi' ? 'en' : (localStorage.getItem('lang') || 'en'),
...
nextTick(async () => {
  ...
  chatSrc.value =
    'https://' + host + '/chat/#/h5/message/blue?' + qs.stringify(dataObj)
  console.log('chatSrc:', chatSrc.value)
})
const onClickLeft = () => {
  // 返回
  router.go(-1)
