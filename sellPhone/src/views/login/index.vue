<template>
  <div
    :style="{ 'background-image': 'url(' + contentBg.href + ')' }"
    class="login-register-content"
  >
    <number-code-dialog v-model="numberCodeVisible" @done="vcodeSuccess" />
    <agree-dialog v-model="agreeVisible" @done="agreeChecked = true" />
    <Vcode
      :successText="t('vertifyPass')"
      :failText="t('vertifuFail')"
      :sliderText="t('vertifyTips')"
      :imgs="vcodeImgs"
      :show="vcodeShow"
    ...
    <div class="header-content">
      <h2>{{ t('login') }}</h2>
      <div class="service">
        <div @click="openService(false)">
          <img src="@/assets/image/shop/service.png" />
        </div>
        <div
          class="language"
    ...
    <div class="form-content">
      ...
      <div v-if="activeType === 1" class="item">
        ...
        <div class="input-content">
          <input-item
            v-model="formData.username"
            v-model:codeNum="codeNum"
            type="tel"
            :placeholder="t('entryPhone')"
            clear
            area-code
          />
        </div>
        <!-- <div class="title" style="margin-top: 20px;">验证码</div>
		<div class="input-content">
      ...
      <div class="link-content" :class="{ 'is-ar': isArLang }">
        <p>
          {{ t('如果您没有账号') }},
          <span class="link" @click="goRegister">{{ t('点击注册') }}</span>
        </p>
      ...
      <div
        ...
        :class="{ 'is-ar': isArLang }"
      >
        <div class="check" @click="agreeChecked = !agreeChecked">
          <van-icon v-if="agreeChecked" name="success" :size="12" />
        </div>
        <p>
          <span @click="agreeChecked = !agreeChecked">{{
            t('我已阅读并同意')
          }}</span
          ><span class="link" @click="agreeVisible = true"
            >《{{ t('用户协议') }}》</span
          >
        </p>
      </div>
...
if (mode === 'familyShop') {
  contentBg.value = new URL(
    '@/assets/image/login/bg_familyShop.png',
    import.meta.url
  )
}
...
const arrowIcon = new URL(
  '@/assets/image/icon/icon_arrow_down.png',
  import.meta.url
)
...
const storeAreaCode = localStorage.getItem('areaCode')
...
const sendCode = () => {
  console.log('发送验证码', formData.username)
}
...
const vcodeSuccess = () => {
  ...
  newLoginUser({
    username:
      activeType.value == 0
        ? formData.username
        : `${codeNum.value} ${formData.username}`,
...
const goRegister = () => {
  ...
  const href =
    hostname === 'localhost'
      ? 'https://tiktokmall666.com/promote/#/'
      : `${origin}/promote/#/`
