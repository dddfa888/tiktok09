    ...
    <div class="list-content">
      <div class="item-list" @click="openSystemList">
        ...
        <div class="flex txt-content">
          ...
          <p class="time">
            {{ formatZoneDate(sysMsgInfo.time, 'YYYY-MM-DD HH:mm') }}
          </p>
        </div>
      </div>
      <div
      ...
      >
        <div class="avatar">
          <img
            :src="
              item.avatarImg || (isShop ? userDefault.href : shopDefault.href)
            "
            alt=""
          />
        </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const shopDefault = new URL(
      '@/assets/image/avatar/avatar_d.png',
      import.meta.url
    )
    const userDefault = new URL(
      '@/assets/image/userAvatar/default.png',
      import.meta.url
    )
    const listData = ref([])
    ...
    const dataFormate = async (data) => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        const localUpdatetime = formatZoneDate(
          obj.updatetime,
          'YYYY-MM-DD HH:mm'
        )
        const updatetime = new Date(localUpdatetime).getTime()
        const updatetimeStr = localUpdatetime.slice(
          localUpdatetime.indexOf(' ') + 1
        )
        ...
        if (isShop.value) {
          ...
          if (data[i].useravatar.length < 5) {
            if (data[i].useravatar && !isNaN(Number(data[i].useravatar))) {
              await import(
                `./../../../assets/image/userAvatar/${data[i].useravatar}.png`
              ).then((res) => {
            ...
            obj.avatarImg = data[i].useravatar
            dataArr.push(obj)
          }
        } else {
      ...
      messagePagelist(params).then((res) => {
        ...
        if (resData.length) {
          ...
          dataArr.forEach((item) => {
            varObj[item.code] = ['complaintReason'].includes(item.code)
              ? t(item.value)
              : item.value
          })
          ...
          let resTxt = ''
          if (
            ['inbox_recharge_success', 'inbox_withdraw_success'].includes(
              data.bizType
            )
          ) {
            const tMsg =
              data.bizType === 'inbox_recharge_success'
                ? 'rechargeSuccessTips'
                : 'withdrawalSuccessTips'
            resTxt = t(tMsg, { orderAmount: getValueStr(data, 'orderAmount') })
          } else if (
            ['inbox_store_audit_fail', 'inbox_store_audit_success'].includes(
              data.bizType
            )
          ) {
            resTxt =
              data.bizType === 'inbox_store_audit_fail'
                ? t('storeAuthenticationFailedTips', {
                    shop_name: getValueOf(data, 'shop_name'),
                    reason: getValueOf(data, 'reason')
                  })
