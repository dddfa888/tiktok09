      ...
      >
        ...
        <ul ref="msgTxtContent" class="flex flex-col pt-3">
          <li
            ...
            :key="item.id"
            :class="!item.delete_status ? '' : 'flex flex-col my-3'"
            
          >
            <p
              class="font-13 text-center py-2 text-grey"
              v-if="showTime(index) && !item.delete_status"
            >
              {{ formatZoneDate(item.createtime, 'YYYY-MM-DD') }}
            </p>

            <div
            v-if="!item.delete_status"
...
import { _getMsg, _getUnreadMsg, _sendMsg,_getOnlineTime } from '@/service/im.api'
  ...
  try{
   nextTick(() => {
    ...
    if(res){
      if(!res.oline){
        Toast({
          message: t("customer_service_time", { start: res.start, end: res.end }),
          duration: 10000
        });
      }
    }
