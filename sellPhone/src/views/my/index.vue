    ...
    <div class="content">
        ...
        <div class="flex items-center userinfo-content">
          ...
          <div class="text-white">
            ...
            <div class="flex items-center">
              <div class="text-sm" :class="isArLang ? 'ml-1' : 'mr-1'">ID: {{ userInfo.usercode }}</div>
      ...
      <div v-if="nextLevelObj" class="level-progress">
        <!-- <div
          ...
          :style="{
            'background-image':
              'url(' + getLevelIconImg(currentLevelObj.name) + ')'
          }"
        >
        ...
        <!-- <div
          ...
          :style="{
            'background-image':
              'url(' + getLevelIconImg(nextLevelObj.name) + ')'
          }"
        >
          {{ nextLevelObj.name }}
        </div>-->
      ...
      <div class="level-number-content">
        <!-- <p
          ...
          v-html="
            t('当前运行资金，距离等级还需', {
              money: numberStrFormat(storeMoneyRechargeAcc),
              level: nextLevelObj.name,
              money1: numberStrFormat(
                Number(nextLevelObj.rechargeAmountCnd) -
                  Number(storeMoneyRechargeAcc)
              )
            })
          "
        ></p>-->
        <!-- <div v-if="currentLevelObj" @click="openPage('/sellerLevel')">
          <p>
            ...
            ><span v-if="currentLevelObj.popularizeUserCountCnd" class="em"
              >/{{ currentLevelObj.popularizeUserCountCnd }}</span
            >, {{ t('当前团队人数') }}<span>{{ currentTeamNum }}</span
            ><span v-if="currentLevelObj.teamNum" class="em"
              >/{{ currentLevelObj.teamNum }}</span
            >, {{ t('当前采购优惠') }}
            <span>{{ currentLevelObj.sellerDiscount }}</span>
          </p>
              ...
              <div>
                <div class="flex items-center freeze-total-content">
                  <div class="font-bold text-xl" :class="isArLang ? 'ml-3' : 'mr-3'">
                    ...
                    showWalletNum
                    ? `$${numberStrFormat(
                    userInfo.money + userInfo.frozen
                    )}`
                    : '*****'
                    }}
                  </div>
                  <i
                    v-if="showWalletNum"
                    class="iconfont icon-denglu-mimakejian"
                    @click="showWalletNum = false"
                  ></i>
                  <i
                    v-if="!showWalletNum"
                    class="iconfont icon-denglu-mimabukejian"
                    @click="showWalletNum = true"
                  ></i>
                  <i class="iconfont icon-shuaxin" @click="refresh"></i>
                </div>
                <div class="freeze-num-content">
                  <!-- {{ t('balance') }} -->
                  <!-- <p v-if="Number(userInfo.lateFee)">
                    (<span
                      >{{ t('待到账金额') }}
                      {{
                        showWalletNum
                          ? `$${numberStrFormat(userInfo.lateFee)}`
                          : '*****'
                      }}</span
                    >)
                  </p>-->
                  <!-- <p v-if="Number(userInfo.frozenAndLate)">
                    (<span
                      >{{ t('送货金额') }}
                      {{
                        showWalletNum
        ...
        <van-swipe class="my-swipe" :autoplay="5000" indicator-color="white">
          <van-swipe-item v-if="showRward && rwardsInfo.length">
            <div class="first-content">
              ...
              >{{ t('去充值') }}</div>
              <div
                v-if="rechargeBonusStatus && showRawrdBtn"
                class="btn get"
                @click="rawrdHandle"
              >{{ t('领取') }}</div>
            </div>
          ...
          <van-cell is-link :value="currentLang" to="/language">
            <template #title>
              <div class="flex items-center">
                ...
                <div class="color-333" :class="isArLang ? 'mr-2' : 'ml-2'">{{ t('language') }}</div>
              </div>
            </template>
          </van-cell>
          ...
          <van-cell is-link to="/finance">
            <template #title>
              <div class="flex items-center">
                ...
                <div
                  class="color-333"
                  :class="isArLang ? 'mr-2' : 'ml-2'"
                >{{ t('financialStatement') }}</div>
              </div>
            </template>
          </van-cell>
          ...
          <van-cell is-link to="/setting">
            <template #title>
              <div class="flex items-center">
                ...
                <div class="color-333" :class="isArLang ? 'mr-2' : 'ml-2'">{{ t('setting') }}</div>
              </div>
            </template>
          </van-cell>
          ...
          <van-cell is-link @click="logout">
            <template #title>
              <div class="flex items-center">
                ...
                <div class="color-333" :class="isArLang ? 'mr-2' : 'ml-2'">{{ t('退出登录') }}</div>
              </div>
            </template>
          </van-cell>
        </van-cell-group>
...
import { useShopInfoStore } from '@/store/user' // 注意路径是否为 user.js
...
const bgImg = getImg(`imgs/footer/${colorMode}/bg1.png`)
...
const getActivityInfo = async () => {
  await sellerInfo().then((res) => {
    ...
    let obj = { shopInfo: res }
    localStorage.setItem('shopInfo', JSON.stringify(obj))
  })
  ...
  malllevelList().then((res) => {
    ...
    if (data.length) {
      ...
      currentLevelObj.value =
        levelDataRef.value.find((item) => item.name === currentLevel.value) ||
        defaultLevel
      const currentLevelObjIndex = levelDataRef.value.findIndex(
        (item) => item.name === currentLevel.value
      )
      ...
      if (nextLevelObj.value) {
        shopInfo.creditScore.value =
          Number(storeMoneyRechargeAcc.value) /
            Number(nextLevelObj.value.rechargeAmountCnd) >
          1
            ? '100%'
            : Number(
                numberStrFormat(
...
const goToLoan = () => {
  ...
  const href =
    hostname === 'localhost'
      ? 'http://localhost:8022/#/loan'
      : `${origin}/wap/#/loan`
  window.location.href = `${href}?lang=${locale.value}&token=${userStore.userInfo.token}`
}
