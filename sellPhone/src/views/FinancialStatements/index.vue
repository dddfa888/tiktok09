  ...
  <div>
    ...
    >
      ...
      <van-list
        ...
        :finished="finished"
        :loading-text="t('loading')"
        :finished-text="listData.length ? t('product.3') : ''"
        @load="getListData"
      >
        <div v-if="listData.length" class="list-content">
          <div v-for="(item, index) in listData" :key="index" class="item">
            ...
            <div class="amout">
              ...
              <div class="profit">
                {{ t('利润') }} {{ numberStrFormat(item.totalProfit) }}
              </div>
            </div>
          </div>
        </div>
...
import { sellerReportHead, reportList } from '@/service/shop.api.js'
...
export default defineComponent({
  ...
  setup() {
    ...
    const filterIcon = new URL(
      '@/assets/image/fundsRecords/Frame1277.png',
      import.meta.url
    )
    const imgObj = {
      calendar: new URL('@/assets/image/finance/calendar.png', import.meta.url),
      credit: new URL(
        '@/assets/image/finance/credit-card.png',
        import.meta.url
      ),
