<template>
  <div class="home" id="home">
    <EsHeaderView :token="userInfo?.username" />
    ...
    >
      <div class="download">
        ...
        <div
          class="btn-content"
          v-if="itemname !== 'Hive' && itemname !== 'TikTok-Wholesale'"
        >
...
import { UserInfoApi } from "@/api/common.js";
...
export default {
    ...
    return {
      ...
      banner1:
        process.env.VUE_APP_ITEM_NAME == "FamilyShop"
          ? require("@/assets/image/familyShop_banner.png")
    ...
    "$route.query.token": {
      ...
      handler(token) {
        if (token) {
          ...
          // 构造不带 token 的 query 并跳转
          const { query, path } = this.$route;
          const newQuery = { ...query };
          delete newQuery.token;

          // 替换路由（不刷新页面，但会重新触发 watch 和组件逻辑）
          this.$router.replace({ path, query: newQuery }).then(() => {
            this.reloadComponent(); // 手动重新加载组件
          });
        }
      },
    },
  ...
  created() {
    ...
    if (token && !tokenStored) {
      ...
      // 获取当前完整的 hash 路径（比如 #/home?token=xxx）
      const hash = window.location.hash;

      // 去掉 token 参数（可能还有其他参数）
      ...
      window.location.replace(
        window.location.origin + window.location.pathname + cleanedHash
      );
    }
  },
  activated() {
    ...
    async fetchUserInfo() {
      try {
        ...
        if (res && res.data) {
          ...
          console.log("成功写入Vuex：", this.$store.state.userInfo);
        }
      } catch (error) {
