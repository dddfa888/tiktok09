<template>
  ...
  <div :class="['header', isStore && 'no-gap']">
    <div
      ...
      :style="
        !headerFlag
          ? 'padding:0 !important; transition: padding .5s linear 0s; background: rgba(255, 255, 255, 0.95); box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); height:auto !important;'
          : ''
      "
    >
      <div class="header-top flex-between app-container">
        <div
          class="header-logo flex-start"
          @click="goto('/')"
          :style="
            itemname == 'TikTok'
              ? 'height:78px'
        ...
        >
          <img
            :src="
              itemname == 'TikTok' || itemname == 'FreshCo'
                ? require(`@/assets/image/${itemname}/shoplogo.png`)
                : itemname == 'TikTok-Wholesale'
                ? require(`@/assets/image/${itemname}/${itemname}logo.svg`)
                : require(`@/assets/image/${itemname}/shoplogo.svg`)
            "
            :style="
              itemname == 'SM-wholesale shop'
                ? 'width:220px;margin-right:auto'
                : itemname == 'TikTok'
                ? 'height:38.2px ;width:160px '
                : itemname == 'TikTok-Wholesale' || itemname == 'FreshCo'
                ? 'width:200px'
                : 'width:100%'
            "
          />
        ...
        <div class="header-search" slot="reference">
              ...
              <div class="search-content">
                ...
                <div class="search-content-history">
                  ...
                  <div class="search-content-history-list">
                    ...
                    <div v-else class="empty">{{ $t("message.home.noData" /**暂无数据 */) }}</div>
                  </div>
                </div>
              </div>
            </template>
          ...
          >{{ $t("message.home.search" /**搜索 */) }}</el-button>
        </div>
        <div class="header-user">
          <div class="flex-start">
            ...
            <div class="no-login">
              ...
              <!-- <span>{{ $t("message.home.or") }}</span> -->
              <span class="text" @click="handleLogout">{{ $t("message.home.logout" /**退出 */) }}</span>
            </div>
            <div style="margin-right: 20px">
              <el-badge v-if="!!unPtcount" :value="unPtcount" @click.native="openService">
    ...
    <div v-if="show" class="mask" @click.self="close">
      <div class="box">
        <h3 class="title">{{ $t("message.home.customeService") }}</h3>

        <div class="item" @click="select('one')">{{ $t("message.home.service") }}1</div>
        <div class="item" @click="select('two')">{{ $t("message.home.service") }}2</div>

        <button class="close-btn" @click="close">{{ $t("message.home.close") }}</button>
      </div>
    </div>
  </div>
  <!-- </el-header> -->
...
export default {
  ...
  methods: {
    ...
    // 获取客服在线时间
    async getCustomerServiceTime() {
      try {
        let res = await apiGetCustomerServiceTime();
        //  this.$message.warning(this.$t("message.home.customer_service_time", { start: res.data.start, end: res.data.end }));
        if (res.code == 0) {
          this.$alert(
            this.$t("message.home.customer_service_time", {
              start: res.data.start,
              end: res.data.end
            }), // 内容
            this.$t("message.home.hint"), // 标题
            {
    ...
    select(name) {
      if (name === "one") {
        window.open(this.onlinePath.split(",")[0], "_blank");
      } else {
        window.open(this.onlinePath.split(",")[1], "_blank");
      }
      this.close();
    },
    openService() {
    ...
    goToSearchRes(flag) {
      ...
      if (this.$route.name !== pageName) {
        ...
        this.$router.replace(
          {
            path: `${pageUrl}/?k=${encodeURIComponent(
              JSON.stringify(String(keyword))
            )}`
          },
          () => {
            location.reload();
