...
export function openChatPage(token = "", partyId = "", name = "", productId = "") {
  ...
  let lang=getCurrentLang().lang
  if(lang=='vi'){
    lang='en'
  }
  window.open(`${config.HOST_URL}/chat/#/pc/yellow?token=${token}&partyid=${partyId}&name=${name}&lang=${lang}&selfimg=${avatar}&productId=${productId}`, '_blank')
