    ...
    <div v-if="isShelves" class="footer">
      <div class="top">
        <img
          ...
          alt
          :style="
            itemData.customer_service && {
              height: itemData.customer_service + 'px',
            }
          "
          class="logo"
        />
      ...
      <div class="argos">
        ...
        <div :class="['text', isCn && 'cn_text']" v-if="name !== 'TikTokMall'">{{ introduce }}</div>
        <div
          :class="['text', isCn && 'cn_text']"
          v-else
        >{{ $t('TikTok Mall是全球短视频社交平台TikTok与亚洲最大的批发贸易公司Alibaba合作成立的，面向全球的电子商务平台，并在全球拥有三大自营智能仓库分别位于加拿大多伦多、美国洛杉矶及日本千叶。App内在售商品包含来自日本、韩国、中国等亚洲人气商品，目前总数已超过50,000款，并在持续快速增长中。') }}</div>
        <div
          class="bank_card"
        ...
        <div
          :class="['images', name === 'Tongda' && 'amazon', name]"
          v-if="!['Hive', 'TikTok-Wholesale', 'GreenMall', 'INT Overstock'].includes(name)"
        >
          <img
            v-for="(item, k) of argosImages"
            :key="k"
            :src="item.logo"
            v-show="item?.logo"
            alt
      ...
      <van-dialog v-model="dialogShow" :title="$t('联系我们')">
        <div class="dialog_message" v-if="type == 'popover'">
          <div v-if="itemData?.phone">
            <span>{{ $t("企业电话") }}:</span>
            <span>{{ itemData?.phone }}</span>
          </div>
          <div v-if="itemData?.mail">
            <span>{{ $t("企业邮箱") }}:</span>
            <span>{{ itemData?.mail }}</span>
          </div>
          <div v-if="itemData?.mail2">
            <span>{{ $t("企业邮箱") }}2:</span>
            <span>{{ itemData?.mail2 }}</span>
          </div>
          <div v-if="itemData?.telegram">
            <span>Telegram:</span>
            <span>{{ itemData?.telegram }}</span>
          </div>
          <div v-if="itemData?.line">
            <span>Line:</span>
            <span>{{ itemData?.line }}</span>
          </div>
          <div v-if="itemData?.whatsApp">
            <span>WhatsApp:</span>
            <span>{{ itemData?.whatsApp }}</span>
          </div>
          <div v-if="!itemData?.mail">
            <span>{{ $t("企业邮箱") }}:</span>
...
export default {
      ...
      textList: [
        {
          ...
          subset: [
            {
              path: this.$store.state.multiItem[process.env.VUE_APP_ITEM_NAME]
                .customer_service
  ...
  computed: {
    introduce() {
      ...
      const text =
        "全球站的用户来全球103个国家，使用USDT/ETH/BTC进行 结算。USDT/ETH/BTC是一种无国界的交易方式，可以在全球范 围内实现即时低成本交易，无需等待，没有国际费用。";
      if (name === "INT Overstock") {
        const OverstockText =
          "全球站的用户来全球103个国家，使用USDT/USDC/ETH/BTC进行结算。USDT/USDC/ETH/BTC是一种无国界的交易方式，可以在全球范围内实现即时低成本交易，无需等待，没有国际费用。";
  ...
  created() {
    ...
    console.log(this.$store.state.multiItem[process.env.VUE_APP_ITEM_NAME]);
  ...
  methods: {
    ...
    async subscription() {
      if (!this.isValidEmail(this.email))
        return this.$notify(this.$t("请输入正确的邮箱地址"));
