    ...
    methods: {
        ...
        async getCustomerService() {
            ...
            if (customer_service_url) {
                const { multiItem } = this.$store.state;
                const itemName = this.itemName;

                // 如果对应的 item 不存在，先初始化为一个对象
                if (!multiItem[itemName]) {
                    // ⚠️ 直接修改 Vuex 的 state 是不推荐的，这里只是防止报错
                    // 正确做法见下方 Vuex 建议
                    this.$set(multiItem, itemName, {});
                }

                multiItem[itemName].customer_service = customer_service_url;
            }
        }
        ,
        handleSetLang(lang) {
