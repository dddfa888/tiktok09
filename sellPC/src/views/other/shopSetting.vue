  ...
  <div class="merchandise">
        ...
        <div class="info">
          ...
          <div class="info-m">
            <el-form-item :label="$t('店铺名称')">
              <el-input
                v-model="paramsData.basicInfo.shopName"
            ...
            <el-form-item :label="$t('联系人')">
              <el-input
                v-model="paramsData.basicInfo.shopUserName"
            ...
            <el-form-item :label="$t('店铺电话')">
              <el-input
                v-model="paramsData.basicInfo.shopPhone"
                :placeholder="$t('请输入店铺电话')"
                :class="$store.getters.currentLanguage === 'en' ? '' : 'active'"
                @blur="updateMerchantInfo"
              ></el-input>
            </el-form-item>
            <el-form-item :label="$t('店铺简介')">
              <el-input
                maxlength="200"
                v-model="paramsData.basicInfo.shopIntr"
                :disabled="false"
                :placeholder="$t('请输入店铺简介')"
                class="desc"
                type="textarea"
                :class="
                  $store.getters.currentLanguage === 'en'
                    ? 'el-input'
                    : 'el-input active'
                "
                @blur="updateMerchantInfo"
            ...
            <el-form-item :label="$t('进店欢迎语')">
              <el-input
                maxlength="200"
                v-model="paramsData.basicInfo.imInitMessage"
                :disabled="false"
        ...
        <div class="info">
          ...
          <div class="banner-settings">
            <div style="margin-bottom: 15px">
              <p class="mb-15 font-14 font-400 text-black mb-4">
                {{ $t("店铺横幅") }} 1(1920x300)
              </p>
              <Upload
        ...
        <div class="info">
          ...
          <div class="info-m">
            <el-form-item label="Facebook">
              <el-input
                v-model="paramsData.sociality.Facebook"
                :placeholder="$t('使用https插入链接')"
              ></el-input>
              <el-button
                style="margin-left: 10px"
                type="primary"
                @click="updateMerchantInfo($event)"
                >{{ $t("确定") }}</el-button
              >
            </el-form-item>
            <el-form-item label="Twitter">
              <el-input
                v-model="paramsData.sociality.Twitter"
                :placeholder="$t('使用https插入链接')"
              ></el-input>
              <el-button
                style="margin-left: 10px"
            ...
            <el-form-item label="Google">
              <el-input
                v-model="paramsData.sociality.Google"
                :placeholder="$t('使用https插入链接')"
              ></el-input>
              <el-button
                style="margin-left: 10px"
            ...
            <el-form-item label="YouTube">
              <el-input
                v-model="paramsData.sociality.YouTube"
                :placeholder="$t('使用https插入链接')"
              ></el-input>
              <el-button
                style="margin-left: 10px"
            ...
            <el-form-item label="Instagram">
              <el-input
                v-model="paramsData.sociality.Instagram"
                :placeholder="$t('使用https插入链接')"
              ></el-input>
              <el-button
                style="margin-left: 10px"
      ...
      <el-card>
        <div class="info">
          ...
          <div class="info-m">
            ...
            <!-- <el-form-item
              :label="$t('资金密码')"
              v-if="!defaultSettings.hideUpdatePayPassword"
            >
              <span
                class="cursor-pointer"
                style="color: #0066ff; margin-left: 10px"
                @click="changePass('修改资金密码')"
                >{{ $t("修改") }}</span
              >
            </el-form-item> -->
            <el-form-item
              :label="$t('登录密码')"
              v-if="!defaultSettings.hideUpdateLoginPassword"
            >
              <span
                class="cursor-pointer"
                style="color: #0066ff; margin-left: 10px"
            ...
            <el-form-item
              :label="$t('注销账号')"
              v-if="['Argos', 'Argos2', 'GreenMall'].includes(projectTitle)"
            >
              <span
                class="cursor-pointer"
    ...
    <el-dialog :title="passTitle" :visible.sync="passVisible" width="450px">
      <div class="dia-main">
        <el-form>
          <el-form-item>
            <p class="font-12 font-400" style="color: #333">
              {{ $t("新密码") }}
            </p>
      ...
      <el-button class="w-full h-44" type="primary" @click="biaodantijiao4">{{
        $t("提交")
      }}</el-button>
    </el-dialog>
    <!--  修改资金密码/修改密码  -->
    <el-dialog :title="passTitle" :visible.sync="passVisible3" width="450px">
      ...
      <el-button class="w-full h-44" type="primary" @click="biaodantijiao5">{{
        $t("提交")
      }}</el-button>
    </el-dialog>
    <!--  修改登录密码  -->
    ...
    >
      ...
      <el-button class="w-full h-44" type="primary" @click="biaodantijiao3">{{
        $t("提交")
      }}</el-button>
    </el-dialog>
    <!--  修改手机/邮箱  -->
    <UpdateIphoneOrEmail
      ref="updateIphoneOrEmail"
    ...
    >
        ...
        <el-form>
          ...
          <el-form-item>
            ...
            <el-select
              ...
              :disabled="
                kyc_get.status == 3 || kyc_get.status == 0 ? false : true
              "
              :placeholder="$t('请选择国籍')"
              filterable
            >
          ...
          <el-form-item>
            <p class="mb-6 text-black">{{ $t("Yqm") }}</p>
            <!--            <el-input placeholder="请填写真实姓名"/>-->
            <el-input
              v-model="kycForm.yqm"
              :placeholder="$t('entryYqm')"
              clearable
              maxlength="20"
            />
          </el-form-item>
          <el-form-item v-if="kycForm.status !== 2">
            ...
            <div class="shangchaun">
              <div class="c1">
                <van-uploader
                  v-model="kycForm.idimg_1"
                  :after-read="afterRead21"
                  :deletable="
                    kyc_get.status != 3 && kyc_get.status != 0 ? false : true
                  "
                  :max-count="1"
                  :readonly="
                    !(kyc_get.status == 3 || kyc_get.status == 0 ? true : false)
                  "
                  preview-size="105px"
                />
                <div>{{ $t("证件正面") }}</div>
              ...
              <div class="c1">
                <van-uploader
                  v-model="kycForm.idimg_2"
                  :after-read="afterRead22"
                  :deletable="
                    kyc_get.status != 3 && kyc_get.status != 0 ? false : true
                  "
                  :max-count="1"
              ...
              <div class="c1" v-if="!hideCertificationPhoto">
                <van-uploader
                  v-model="kycForm.idimg_3"
                  :after-read="afterRead23"
                  :deletable="
                    kyc_get.status != 3 && kyc_get.status != 0 ? false : true
                  "
                  :max-count="1"
            ...
            <div v-if="kyc_get.status == 3 || kyc_get.status == 0" class="t1">
              {{ $t("拍摄示例") }}
            </div>
            <div
              v-if="kyc_get.status == 3 || kyc_get.status == 0"
              class="shangchaun"
            >
              <div class="c1" style="margin-right: 8px">
                <img
                  class="c1-a-img"
                  :src="require('@/assets/image/auth/01.png')"
                />
              </div>
              <div class="c1" style="margin-right: 8px">
                <img
                  class="c1-a-img"
                  :src="require('@/assets/image/auth/02.png')"
                />
              </div>
              <div class="c1" v-if="!hideCertificationPhoto">
                <img
                  class="c1-a-img"
                  :src="require('@/assets/image/auth/03.png')"
                />
              </div>
            </div>
          </el-form-item>
      ...
      <div
        ...
        style="background: #1552f0; cursor: pointer"
        @click="renzzheng"
      >
    ...
    >
      <div class="update-avatar">
        <div v-for="(item, index) in 20">
          <div class="update-avatar-item" @click="userInfo.avatar = index + 1">
            <el-image
              class="avatar-image"
            ...
            />
            <el-image
              class="update-avatar-true"
              :class="{
                'update-avatar-true-active': userInfo.avatar == index + 1,
              }"
              :src="require(`@/assets/avatar/true.png`)"
              alt=""
            />
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    renzzheng() {
      ...
      if (
        t.kycForm.idimg_2.length == 0 ||
        t.kycForm.idimg_1.length == 0 ||
        (!this.hideCertificationPhoto && t.kycForm.idimg_3.length == 0)
      ) {
        Toast(t.$t("请完善证件照片上传"));
        return;
      }
      ...
      const data = {
        ...
        idimg_3: !this.hideCertificationPhoto
          ? this.shangchuanurl3 || t.kycForm.idimg_3[0].url
          : null,
      };
    ...
    getMerchantInfo(cb) {
      ...
      _getIdentify({}).then((res) => {
        ...
        if (res.status === 3) {
          this.kycForm.idimg_1 = res.data.idimg_1_path
            ? [{ url: res.data.idimg_1_path }]
            : [];
          this.kycForm.idimg_2 = res.data.idimg_2_path
            ? [{ url: res.data.idimg_2_path }]
            : [];
          this.kycForm.idimg_3 = res.data.idimg_3_path
            ? [{ url: res.data.idimg_3_path }]
            : [];
        } else {
          this.kycForm.idimg_1 = res.data.idimg_1_path
            ? [{ url: res.data.idimg_1 }]
            : [];
          this.kycForm.idimg_2 = res.data.idimg_2_path
            ? [{ url: res.data.idimg_2 }]
            : [];
          this.kycForm.idimg_3 = res.data.idimg_3_path
            ? [{ url: res.data.idimg_3 }]
            : [];
        }
        if (res.data.status === 2) {
          //屏蔽敏感信息
          this.kycForm.idnumber = this.hidePassportNumber(
            this.kycForm.idnumber
          );
          //屏蔽name
    ...
    changeInfo(str) {
      switch (str) {
        case 1:
          this.$refs.updateIphoneOrEmail.changeEditTitle(
            this.$t("修改手机"),
            str
          );
          break;
