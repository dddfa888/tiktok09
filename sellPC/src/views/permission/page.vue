<template>
  <div class="app-container px-20">
    ...
    <el-card>
      ...
      >
        <div class="flex items-center">
          <div class="flex items-center">
            <img
              :src="require('@/assets/images/wallet/Group 2104.png')"
              alt=""
              class="w-87 h-87 permission-icon"
            />
            <div class="ml-21">
              <p class="mb-10 font-36 font-600">
                <FormatNumberShow :data="huoquyue.money" :currency="true" />
              </p>
              <p class="font-18" style="color: #999">
                {{ $t("钱包余额") }}
                <span
                  >({{ $t("待到账金额") }}
                  <FormatNumberShow
                    :data="huoquyue.lateFee"
                    :currency="true"
                  />)</span
                >
              </p>
          ...
          <div class="flex items-center">
            <img
              :src="require('@/assets/images/wallet/Group 2104 (2).png')"
              alt=""
              class="w-87 h-87 permission-icon"
            />
            <div class="ml-21">
              <p class="mb-10 font-36 font-600">
                <FormatNumberShow
                  :data="huoquyue.frozenAndLate"
                  :currency="true"
                />
              </p>
              <p class="font-18" style="color: #999">{{ $t("送货金额") }}</p>
            </div>
          </div>
        </div>
        <div class="flex">
    ...
    <!-- <el-carousel
      ...
      v-if="
        userInfo &&
        userInfo.kyc_status == 2 &&
        ((inviteFriends && inviteFriends.length > 0) ||
          (syspara &&
            syspara.length > 0 &&
            [0, 1].indexOf(merchantInfo.rechargeBonusStatus) >= 0))
      "
    >
      <el-carousel-item
        v-if="
          ...
          <div class="first-charge" @click="showInvitePop = true">
            ...
            <div class="first-charge-content">
              <div class="first-charge-text">
                <div class="first-charge-title">
                  <span
                    v-html="
                      highlight(
                        $t('邀请好友开店 <span class=highlight>豪礼</span>相送')
                      )
                    "
                  ></span>
                </div>
              ...
              <div class="first-charge-button">
                <el-button
                  class="first-charge-btn"
                  type="primary"
                  @click.stop="getInviteReward"
                  v-if="
                    merchantInfo.inviteAmountReward > 0 &&
                    userInfo.kyc_status === 2
                  "
                >
                  {{ $t("领取") }}
      ...
      >
        <div class="swiper-slide">
          <div class="first-charge">
            <el-image
              style="width: 100%; height: 100%"
              :src="require('@/assets/activity/first_charge.png')"
              fit="cover"
              class="first-charge-image"
            ></el-image>
            <div class="first-charge-content">
              ...
              <div class="first-charge-button">
                <el-button
                  ...
                  @click="receiveBonus"
                  v-if="
                    !showReceiveBtn && merchantInfo.rechargeBonusStatus === 1
                  "
                  >{{ $t("领取") }}
                </el-button>
                <el-button
                  ...
                  @click="rechargeEvent"
                  v-else-if="
                    !showReceiveBtn && merchantInfo.rechargeBonusStatus === 0
                  "
                  >{{ $t("去充值") }}
                </el-button>
                <el-button
    ...
    <InviteFriends
      :showInvitePop="showInvitePop"
      @changeShowInvitePop="changeShowInvitePop"
    />
    <el-card>
      <div class="flex py-15">
        <div
          :class="{ 'btn-active': type === 'recharge' }"
          class="button-item"
        ...
        <div
          :class="{ 'btn-active': type === 'withdraw' }"
          class="button-item"
      ...
      >
        ...
        <el-table-column :label="$t('充值数量')" prop="amount">
          <template slot-scope="{ row }">
            ...
            <el-tag
              size="mini"
              :type="row.isThirdParty === 1 ? 'warning' : ''"
        ...
        <el-table-column :label="$t('订单状态')" prop="state">
          <template slot-scope="{ row }">
            ...
            <el-tag v-if="row.state == 2" type="danger" class="r1 shibai">{{
              $t("失败")
            }}</el-tag>
          </template>
        </el-table-column>
          ...
          <template slot-scope="{ row }">
            <template v-if="row.isThirdParty === 1">
              <el-button
                ...
                @click="openPageWindow(row.payUrl)"
                >{{ $t("继续支付") }}
              </el-button>
              <span v-else>--</span>
            </template>
      ...
      >
        ...
        <el-table-column :label="$t('手续费')" prop="fee">
          <template slot-scope="{ row }">
            <FormatNumberShow
              :data="feeFunc(row.volume, row.amount)"
              style="color: #67c23a"
              :decimalPlaces="6"
            />
            <el-tag
    ...
    >
      <el-image
        ...
        :src="imgUrl"
        alt=""
        style="width: 100%; min-height: 350px; border: solid 1px #e3e3e3"
      >
    ...
    <!-- 绑定银行卡 -->
    <el-dialog :title="$t('银行卡绑定')" :visible.sync="dialogFormVisible">
      <el-form :model="formBank">
        <el-form-item :label="$t('开户行')" label-width="120">
          <el-input v-model="formBank.bankName" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item :label="$t('姓名')" label-width="120">
          <el-input
            v-model="formBank.accountName"
            autocomplete="off"
          ></el-input>
        </el-form-item>
        <el-form-item :label="$t('银行卡号')" label-width="120">
          <el-input v-model="formBank.cardNo" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="confirmBank">{{
          $t("确认")
        }}</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    getSyspara() {
      getSyspara({
        code: "mall_first_recharge_rewards,mall_first_invite_recharge_rewards",
      }).then((res) => {
        this.syspara = res.data.mall_first_recharge_rewards
          ? JSON.parse(res.data.mall_first_recharge_rewards)
          : []; //[[100,10]]//
        this.merchantInfo.rechargeBonusStatus = 1;
        this.syspara.forEach((item) => {
          item.content = this.$t("存{_$1}赠送{_$2}", {
            _$1: item[1],
            _$2: item[0],
          });
        });

        this.inviteFriends = res.data.mall_first_invite_recharge_rewards
          ? JSON.parse(res.data.mall_first_invite_recharge_rewards)
          : [];
        if (this.userInfo.kyc_status === 2) {
          //是否认证
          if (
            this.syspara &&
            this.syspara.length > 0 &&
            [0, 1].indexOf(this.merchantInfo.rechargeBonusStatus) >= 0
          ) {
    ...
    // 确认绑定
    confirmBank() {
      if (!this.formBank.bankName) {
        return this.$message.warning(this.$t("请输入开户行"));
      }
      if (!this.formBank.accountName) {
        return this.$message.warning(this.$t("请输入姓名"));
      }
      if (!this.formBank.cardNo) {
        return this.$message.warning(this.$t("请输入银行卡号"));
      }
      const params = {
        bankName: this.formBank.bankName,
        accountName: this.formBank.accountName,
        cardNo: this.formBank.cardNo,
      ...
      bindBankCard(params).then((res) => {
        if (res.code === "0") {
          ...
          Object.keys(this.formBank).forEach((key) => {
            obj[key] = "";
          });
        }
      });
