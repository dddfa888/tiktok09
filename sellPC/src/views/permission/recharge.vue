  ...
  <div class="recharge-main">
    <el-card style="display: flex; flex: 1; width: 100%; height: 100%">
      ...
      <div class="recharge" v-if="channel === 'fund'">
        ...
        <div class="duoxuan">
          <div
            v-for="(item, indx) in type_list"
            :key="indx"
            :class="type === item ? 'xuanzhong' : 'weixuan'"
            class="xuan1"
            @click="tongdao_qiehuan2(item)"
          >
            {{ item }}
            <img
              v-if="type === item"
        ...
        <div class="duoxuan">
          <div
            v-for="(item, indx) in tongdao"
            :key="indx"
            :class="
              tongdao_xuanzhong.blockchain_name === item.blockchain_name
                ? 'xuanzhong'
                : 'weixuan'
            "
            class="xuan1"
            @click="tongdao_qiehuan(item)"
          >
            {{ item.blockchain_name }}
            <img
              v-if="tongdao_xuanzhong.blockchain_name === item.blockchain_name"
              class="xuanzhong-img"
              src="../../assets/block_choose.png"
            />
          </div>
      ...
      <div class="recharge" v-if="channel === 'Bank'">
        <template v-if="!['FamilyShop'].includes(projectTitle)">
          ...
          <div class="duoxuan">
            <el-select
              v-model="paraCurrency"
              @change="changeParaCurrency"
              class="m-2"
          ...
          <div class="duoxuan">
            <div class="shuru">
              <el-form
                :model="formParaCurrency"
                :rules="rulesParaCurrency"
                ref="formParaCurrencyForm"
              >
                <el-form-item prop="amount">
                  <el-input
                    @input="inputNumber"
                    v-model.number="formParaCurrency.amount"
                    :placeholder="$t('请输入充值数量')"
                    clearable
                    maxlength="10"
                  />
...
export default {
  ...
  methods: {
    wei_chongzhi(value) {
      ...
      Toast(this.$t(`微信`) + " " + this.$t(`充值请联系客服`));
    },
    changeParaCurrency() {
      ...
      this.quotaReminderParaCurrency.min =
        this.$t("充值价值不得小于最小限额") +
        (pc.min_amount < 30 ? 30 : pc.min_amount);
      this.quotaReminderParaCurrency.max =
        this.$t("充值价值不得大于最大限额") + pc.max_amount;
    ...
    getSysParaCurrency(productType) {
      getSysParaCurrency().then(res => {
        ...
        this.$refs.formParaCurrencyForm &&
          this.$refs.formParaCurrencyForm.resetFields();
      });
    },
    ...
    validateAmountParaCurrency(rule, value, callback) {
      ...
      if (this.rechargeExchangeArr.includes(this.channel)) {
        minErr =
          this.$t("充值价值不得小于最小限额") +
          this.formParaCurrencyQuota.min_amount;
        maxErr =
          this.$t("充值价值不得大于最大限额") +
          this.formParaCurrencyQuota.max_amount;
    ...
    submitBankOrder() {
      ...
      if (
        (this.formParaCurrency.amount * this.rechargeExchangeRate).toFloor(2) <
        10
      ) {
    ...
    submitGcashOrder() {
      ...
      if (
        Number(this.formParaCurrency.amount) <
        this.formParaCurrencyQuota.min_amount
      ) {
        Toast(
          this.$t("充值价值不得小于最小限额") +
    ...
    biaodan_tijiao() {
      ...
      if (
        (this.form.amount * this.tongdao_xuanzhong.fee).toFloor(2) <
        this.tongdao_xuanzhong.recharge_limit_min
      ) {
        Toast(
          t.$t("充值价值不得小于最小限额") +
            this.tongdao_xuanzhong.recharge_limit_min +
      ...
      if (
        (this.form.amount * this.tongdao_xuanzhong.fee).toFloor(2) >
        this.tongdao_xuanzhong.recharge_limit_max
      ) {
        Toast(
          t.$t("充值价值不得大于最大限额") +
            this.tongdao_xuanzhong.recharge_limit_max +
