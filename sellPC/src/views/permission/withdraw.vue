  ...
  <div class="withdraw-main">
      ...
      <div style="padding-top: 24px; width: 580px">
        ...
        >
          ...
          >
            ...
            <!--            </el-radio-group>-->
            <el-select
              @change="changeOption(formWithdraw.withdrawalMethod)"
              v-model="formWithdraw.withdrawalMethod"
              style="width: 100%"
            >
              <el-option
                v-for="item in withdrawalMethodOptions"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              ></el-option>
          ...
          <template v-if="formWithdraw.withdrawalMethod === 1">
            <el-form-item :label="$t('提现币种')" prop="currencyType">
              ...
              <!--              </el-radio-group>-->
              <el-select
                v-model="formWithdraw.currencyType"
                style="width: 100%"
                :disabled="withdrawOpenData.openWithdrawAddressBinding === 1"
              >
                <el-option
            ...
            <template v-if="!(formWithdraw.currencyType === 'bankCard')">
              <el-form-item :label="$t('提现网络')" prop="linkProtocol">
                ...
                <!--              </el-radio-group>-->
                <el-select
                  v-model="formWithdraw.linkProtocol"
                  style="width: 100%"
                  :disabled="withdrawOpenData.openWithdrawAddressBinding === 1"
                >
                  <el-option
              ...
              <el-form-item :label="$t('提现地址')" prop="walletAddress">
                <el-input
                  v-model="formWithdraw.walletAddress"
              ...
              <el-form-item :label="$t('提现金额')" prop="account">
                <div style="position: relative">
                  <el-input
                    v-model="formWithdraw.account"
                    clearable
                    :placeholder="quotaReminder"
                    :disabled="!currencyFee"
                    @input="truncateInput"
                  ></el-input>
                  ...
                  <!-- <el-tag
                    size="mini"
                    class="tip-tag"
                    v-if="!['USDT', 'usdt'].includes(formWithdraw.currencyType)"
                  >
                    <span>≈</span>
                    <span>
                      <FormatNumberShow
                        :data="userBalance / (this.currencyFee | 1)"
                        :decimalPlaces="6"
                      />
                  ...
                  <!-- <div class="doc-tag">
                    <el-tag
                      :type="
                        !['USDT', 'usdt'].includes(formWithdraw.currencyType)
                          ? ''
                          : 'success'
                      "
                      style="margin: 0 4px"
              ...
              <el-form-item :label="$t('实际到账')">
                <div style="position: relative">
                  <el-input
                    v-model="formWithdraw.actualAccount"
                    disabled
                  ></el-input>
                  <div
                    class="doc-tag"
                ...
                <el-tag type="success" size="mini">
                  <span>&nbsp;≈&nbsp;</span>
                  <FormatNumberShow
                    :data="formWithdraw.actualAccount * (this.currencyFee | 1)"
                    :decimalPlaces="6"
                  />
                  USDT
                </el-tag>
              ...
              <el-form-item :label="$t('手续费')">
                <div style="position: relative">
                  <el-input
                    v-model="formWithdraw.feeAccount"
                    disabled
                  ></el-input>
                  <div class="doc-tag">
                    <el-tag type="success"> {{ feeAcc }}% </el-tag>
                    <el-tag type="success" style="margin: 0 4px"> USDT </el-tag>
                  </div>
                </div>
                <el-tag type="success" size="mini">
                  <span>&nbsp;≈&nbsp;</span>
                  <FormatNumberShow
                    :data="formWithdraw.feeAccount / (this.currencyFee | 1)"
                    :decimalPlaces="6"
                  />
                  {{ formWithdraw.currencyType }}
                </el-tag>
          ...
          <template v-if="formWithdraw.withdrawalMethod === 2">
            <template v-if="!['FamilyShop', 'JustShop'].includes(projectTitle)">
              ...
              >
                <el-select
                  v-model="formWithdraw.countryName"
                  :placeholder="$t('请选择国家')"
                  style="width: 100%"
                >
                  <el-option
              ...
              <el-form-item
                :label="$t('国际代码')"
                prop="swiftCode"
                v-if="['Argos'].includes(projectTitle)"
              >
                <el-input
                  v-model="formWithdraw.swiftCode"
              ...
              >
                <el-input
                  v-model="formWithdraw.routingNum"
              ...
              >
                <el-input
                  v-model="formWithdraw.accountAddress"
              ...
              >
                <el-input
                  v-model="formWithdraw.bankAddress"
          ...
          <template v-if="formWithdraw.withdrawalMethod === 3">
            <template v-if="!['FamilyShop', 'JustShop'].includes(projectTitle)">
              <el-form-item :label="$t('姓名')" prop="bankUserName">
                <el-input
                  v-model="formWithdraw.bankUserName"
              ...
              <el-form-item :label="$t('支付宝账号')" prop="bankCardNo">
                <el-input
                  v-model="formWithdraw.bankCardNo"
                  :placeholder="$t('请输入账号')"
                ></el-input>
          ...
          <template v-if="formWithdraw.withdrawalMethod === 4">
            <template v-if="!['FamilyShop', 'JustShop'].includes(projectTitle)">
              <!-- <el-form-item :label="$t('姓名')" prop="bankUserName">
                <el-input v-model="formWithdraw.bankUserName" :placeholder="$t('请输入姓名')"
                ></el-input>
              </el-form-item> -->
              <el-form-item :label="$t('微信绑定手机号')" prop="bankCardNo">
                <el-input
                  v-model="formWithdraw.bankCardNo"
                  :placeholder="$t('请输入手机号')"
                ></el-input>
              </el-form-item>
              <el-form-item :label="$t('上传微信收款码（需保存收款码到相册)')">
    ...
    >
      ...
      >
        <el-form-item :label="$t('提现币种')" prop="currencyType">
          <el-select
            v-model="formWithdrawAddress.currencyType"
            :placeholder="$t('请选择')"
            style="width: 100%"
          >
            <el-option
        ...
        <el-form-item :label="$t('提现网络')" prop="linkProtocol">
          <el-select
            v-model="formWithdrawAddress.linkProtocol"
            :placeholder="$t('请选择')"
            style="width: 100%"
          >
            <el-option
        ...
        <el-form-item :label="$t('提现地址')" prop="walletAddress">
          <el-input
            v-model="formWithdrawAddress.walletAddress"
            :placeholder="$t('提现地址')"
          ></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  watch: {
    "formWithdraw.withdrawalMethod"(val) {
      ...
      // 切换提现方式时，重置 currencyType 和 account
      this.formWithdraw.currencyType =
        val === 2 ? "bank" : this.tongdao[0]?.coin || "";
      this.formWithdraw.account = "";

      console.log(this.formWithdraw, "===== 更新后的提现信息 =====");
      if (this.formWithdraw.withdrawalMethod == 2) {
        this.getBankList();
      }

      // 确保 withdrawLimit 已经有数据
      if (!this.withdrawLimit?.data) {
        console.warn("提现限制数据未加载！");
        return;
      }

      console.log(this.withdrawLimit, "===== 提现限制配置 =====");

      // 根据 withdrawalMethod 动态调整提现最小额度
      const withdrawAmountMin =
        val === 3 || val === 4
          ? this.withdrawLimit.data?.withdrawAmountMin2 || 10
          : this.withdrawLimit.data?.withdrawAmountMin || 10;

      const withdrawAmountMax = this.withdrawLimit.data?.withdrawAmountMax || 0;
      this.max = Math.min(this.userBalance, withdrawAmountMax);

      // this.quotaReminder =
      //   this.max <= withdrawAmountMin
      //     ? this.$t("提款范围") + ` ≥ ${withdrawAmountMin} USDT`
      //     : this.$t("提款范围") +
      //       ` ${withdrawAmountMin}-${this.max.toFixed(2)} USDT`;

      // 更新校验规则
      this.rules.account[1] = {
        ...this.rules.account[1],
        max: this.max,
        message: this.quotaReminder,
      };
    ...
    async "formWithdraw.currencyType"(val) {
      if (val === "bankCard") {
        ...
        this.formWithdraw.linkProtocol =
          this.linkProtocol && this.linkProtocol[0];
  ...
  methods: {
    changeOption(e) {
      if (e == 3 || e == 4 || e == 1) {
        this.formWithdraw.bankUserName = "";
        this.formWithdraw.bankCardNo = "";
      }
    },
    // 自定义上传逻辑
    customUploadRequest({ file, onSuccess, onError }) {
      ...
      // 使用 Axios 或其他 HTTP 库上传文件
      imageUpload(formData)
        .then((res) => {
          Toast.success(this.$t("上传成功"));
          console.log(res, "上传成功");
          this.formWithdraw.qdCode = res.data;
        })
        .catch(function (err) {
          console.log(err);
          Toast(this.$t("添加失败"));
      ...
      getBankCard()
        .then((res) => {
          if (res.code == "0") {
            ...
            this.$set(this.formWithdraw, "bankName", res.data.bankName);
            this.$set(this.formWithdraw, "bankUserName", res.data.accountName);
            this.$set(this.formWithdraw, "bankCardNo", res.data.cardNo);
          }
        })
        .catch((err) => {
          console.log(err);
    ...
    truncateInput() {
      if (this.formWithdraw.account !== "") {
        this.formWithdraw.account = this.formWithdraw.account.replace(
          /[^0-9.]/g,
          ""
        );
    ...
    changeAccount() {
      //计算实际到账，保留两位小数，用截取法
      //先转成U
      let usdtAccount = this.$bigDecimal.multiply(
        this.formWithdraw.account,
        this.currencyFee
      );
      //计算手续费要多少usdt
      let feeAccount =
        this.$bigDecimal.multiply(usdtAccount, this.handlingFee) * 1;
      this.formWithdraw.feeAccount = feeAccount.toFloor(
        feeAccount.toString().split(".")[1]?.length < 2
          ? feeAccount.toString().split(".")[1]?.length
          : 2
      );
      //实际到账的usdt = usdtAccount - 手续费
      usdtAccount = this.$bigDecimal.subtract(
        usdtAccount,
        this.formWithdraw.feeAccount
      );
      //再转成用户选择的币种
      let actualAccount =
        this.$bigDecimal.divide(usdtAccount, this.currencyFee) * 1;
      this.formWithdraw.actualAccount = actualAccount.toFloor(
        actualAccount.toString().split(".")[1]?.length < 6
          ? actualAccount.toString().split(".")[1]?.length
          : 6
      );
    },
    huilu_huoqu_post(cb) {
      this.it.input1 = this.it.input1.replace(/[^0-9.]/g, "");
      huilu_huoqu_post({ symbol: this.formWithdraw.currencyType }).then(
        (res) => {
          ...
          if (this.withdrawOpenData.openWithdrawAddressBinding === 1) {
            if (!this.withdrawOpenData.existWithdrawAddress) {
              ...
              if (this.withdrawOpenData.existWithdrawAddress) {
                ...
                this.formWithdraw.linkProtocol =
                  this.withdrawOpenData.chainName;
                this.formWithdraw.walletAddress =
                  this.withdrawOpenData.existWithdrawAddress;
              }
            }
          }
          this.$refs.formWithdraw.clearValidate();
      ...
      return new Promise((resolve, reject) => {
        ...
        getUserBalance().then((balanceRes) => {
          ...
          withdrawLimitConfig().then((configRes) => {
            this.withdrawLimit = configRes;
            const withdrawAmountMin = configRes.data?.withdrawAmountMin2 || 1;
            const withdrawAmountMax = configRes.data?.withdrawAmountMax2 || 0;
            this.max =
              this.userBalance < withdrawAmountMax
                ? this.userBalance
                : withdrawAmountMax;
    ...
    payCallback(password) {
      withdrawSubmit({
        ...
        channel:
          this.formWithdraw.withdrawalMethod === 2
            ? "bank"
            : this.formWithdraw.withdrawalMethod === 3
            ? "zfb"
            : this.formWithdraw.withdrawalMethod === 4
            ? "wx"
            : this.formWithdraw.currencyType,
        ...
        qdCode: this.formWithdraw.qdCode,
      }).then((res) => {
        if (res.code === "0") {
          this.$notify({
          ...
          if (this.formWithdraw.withdrawalMethod === 4) {
            this.$message.warning(this.$t("微信") + this.$t("提现请联系客服"));
          }
