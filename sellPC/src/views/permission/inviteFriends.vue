      ...
      <div class="invite-pop-content">
        ...
        <div class="invite-pop-main">
          ...
          <div class="invite-pop-rule">
            ...
            <div class="invite-pop-rule-title" style="margin-top: 13px">
              <el-image :src="require('@/assets/active/invite_title_icon.png')"
                        style="width: 16px;height: 12px;margin: 0 12px;" fit="fill"/>
              <span>{{ $t("活动规则") }}2</span>
              <el-image :src="require('@/assets/active/invite_title_icon.png')"
            ...
            <div class="invite-pop-text mt-24 mb-12">
              <div style="display: flex; flex-direction: column;">
                <span>{{ $t("活动期间，用户通过分享邀请码邀请新用户注册，满足条件后，即可获得奖励：") }}</span>
                <span>{{ $t("1. 活动时限：15天") }}</span>
                <span>{{ $t("2. 新用户需完成实名认证和上架产品") }}</span>
                <span>{{ $t("3. 被邀用户在平台上完成首单之日起计算，15天内的订单完成后，系统结算该用户的交易总额并向邀请者派发奖励") }}</span>
              </div>
              ...
              <div class="invite-table">
                ...
                <div class="invite-table-row" v-for="(item,index) in timeoutOrderList" :key="index">
                  <div>{{ $t("超时订单") }}&le;{{ item.maxOrders }}</div>
                  <div class="blue">{{ item.percentage }}</div>
                </div>
              </div>

              <div style="display: flex; flex-direction: column;">
                <span> {{ $t("本次活动资金由TikTok支持，活动总资金100,000,000USD,快来瓜分吧！") }}</span>
                <span>{{ $t("资金结束即活动结束，赶快邀请你的朋友加入吧！") }}</span>
              </div>
            </div>
            <div class="invite-history" style="padding:0 ">
