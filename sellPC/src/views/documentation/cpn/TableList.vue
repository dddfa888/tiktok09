    ...
    <div class="table">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-tag
              v-if="parseInt(scope.row.purchStatus) === 0"
              type="danger"
              >{{ $t("待采购") }}</el-tag
            >
        ...
        >
          <template slot-scope="scope">
            <div
              style="position: relative; padding: 4px; box-sizing: border-box"
            >
              <el-dropdown
                size="small"
                split-button
              ...
              >
                ...
                <el-dropdown-menu slot="dropdown">
                  ...
                  <el-dropdown-item
                    command="c"
                    v-else-if="[2, 3, 4, 5].includes(scope.row.status)"
          ...
          <el-table :data="detailData" max-height="250px">
            <el-table-column :label="$t('封面图')">
              <template slot-scope="{ row }">
                <el-image
                  :alt="row.goodsName"
                  :preview-src-list="[row.goodsIcon]"
                  :src="row.goodsIcon"
                  fit="contain"
        ...
        <div class="main-price">
          ...
          <div class="item">
            ...
            <div
              ...
              v-if="!defaultSettings.hideSellerLevel"
            >
              <FormatNumberShow
                :data="buyDetail.systemPrice"
                :currency="true"
              />
              <div v-if="buyDetail.sellerDiscountPrice">
                {{ $t("优惠价") }}
                <FormatNumberShow
                  :data="buyDetail.sellerDiscountPrice"
                  :currency="true"
                  style="color: #f56c6c"
                />
                ，{{ $t("采购优惠")
                }}<span style="color: #f56c6c"
                  >{{ buyDetail.sellerDiscount * 100 }}%</span
                >
              </div>
            </div>
          </div>
    ...
    >
      ...
      <div class="order-top">
        ...
        <div class="main">
          <div class="main-content">
            <div class="left">
              ...
              <div class="item">
                ...
                <div>
                  <span
                    v-if="parseInt(orderInfo.purchStatus) === 0"
                    :style="{
                      color: '#fff',
                      padding: '2px 5px',
                      'background-color': 'red',
          ...
          <div class="main-content border" v-show="showMore">
            <div class="left">
              <div class="item">
                <div>{{ $t("姓名") }}</div>
              ...
              <div class="item">
                ...
                <div>
                  {{
                    orderInfo.goodType == "virtual" ? "--" : orderInfo.address
                  }}
                </div>
              </div>
              <div class="item">
            ...
            <div class="right">
              ...
              <div class="item">
                ...
                <div>
                  {{
                    orderInfo.goodType == "virtual" ? "--" : orderInfo.postcode
                  }}
                </div>
              </div>
            </div>
    ...
    >
      <div v-loading="logisticsLoading">
        ...
        >
          ...
          <p class="info">
            {{ $t("订单")
            }}<span style="color: #2c78f8"> {{ item.orderId }} </span
            >{{ $t(item.tipsTxt) }}
          </p>
        </div>
      </div>
...
<script>
import {
  api_order_info_action_post,
  api_order_listGoods_action_post,
  caigouPost,
  getLogisticsInfo,
} from "@/api/user";
  ...
  methods: {
    ...
    contactsUser() {
      this.$router.push({
        path: "/chat/index",
        query: {
          name: this.orderInfo.contacts,
          partyid: this.orderInfo.partyId,
        },
      });
    },
    changeShowModel(e) {
    ...
    payCallback(password) {
      ...
      caigouPost({ safeword: password, orderId: that.orderId })
        .then((res) => {
          that.orderId = "";
          that.$notify({
            title: this.$t("成功"),
    ...
    payCallback2() {
      const that = this;
      caigouPost({ orderId: that.orderId })
        .then((res) => {
          that.orderId = "";
          that.$notify({
            title: this.$t("成功"),
    ...
    openOrder(row) {
      ...
      api_order_listGoods_action_post({ orderId: row.id }).then((e) => {
        ...
        this.detailData.forEach((item) => {
          if (item.goodType == "虚拟商品") {
            this.orderInfo.goodType = "virtual";
          }
        });
      });
    ...
    onSubmit() {
      ...
      // this.payModalShow = true;
      // this.settingSafewordFunc();
      let me=this;
      caigouPost({orderId:this.orderId}).then((res) => {
        // 提示
        Toast.success("订单采购成功");
        me.search()
        console.log(res);
      });
