  ...
  <div class="table-list">
    ...
    <el-card>
      <div class="btn">
        ...
        <el-button type="primary" @click="batchEdit($t('批量修改'))">
          {{ $t("批量修改") }}
        </el-button>
        <!-- <el-button
          type="danger"
          v-if="![].includes(setting.projectTitle)"
              ...
              <div class="merchandise-label-item">
                <!-- <div class="button" v-if="![].includes(setting.projectTitle)">
                  <el-switch
                    ...
                    @change="api_goods_update(scope.row)"
                  >
                  </el-switch>
                </div> -->
                <div class="button">
                  <el-switch
                    v-model="scope.row.recTime"
                    :active-value="1"
                    :inactive-value="0"
                    active-color="#13ce66"
                    inactive-color="#ff4949"
        ...
        <el-table-column :label="$t('利润')" align="center" prop="change_money">
          <template slot-scope="scope">
            <div
              :style="{
                color:
                  (scope.row.sellingPrice - scope.row.systemPrice || 0) > 0
                    ? '#409EFF'
                    : '#F56C6C',
              }"
            >
          ...
          <template slot-scope="scope">
            <div style="display: flex; justify-content: center">
              ...
              <!-- <el-button
                ...
                v-if="![].includes(setting.projectTitle)"
                @click="
                  (delVisible = true), (shachu = []), (shachu = [scope.row.id])
                "
                >{{ $t("删除") }}
              </el-button> -->
            </div>
    ...
    <el-dialog :title="$t('删除提示')" :visible.sync="delVisible" width="450px">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button
          type="primary"
          @click="api_order_listGoods_delete(shachu, true)"
        >
          {{ $t("确定") }}
        </el-button>
    ...
    <el-dialog :title="batchTitle" :visible.sync="batchVisible" width="450px">
      <p>
        <span>{{ $t("确定") }}</span>
        {{ batchTitle }}
        <span style="color: red">{{ shachu.length }}</span
        ><span>{{ $t("个商品吗") }}</span>
      </p>
      <span slot="footer" class="dialog-footer">
        ...
        <el-button @click="(batchVisible = false), (shachu = [])">{{
          $t("取消")
        }}</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    xiugai(e) {
      ...
      this.commodityInfo.discountRatio = e.discountRatio
        ? this.$bigDecimal.multiply(e.discountRatio, 100)
        : "";
      this.commodityInfo.shoujia = this.$bigDecimal.subtract(
        e.discountPrice ? e.discountPrice : e.sellingPrice,
        e.systemPrice
      );
    ...
    editVisibleClose() {
      this.commodityInfo.profitRatio = this.$bigDecimal.divide(
        this.commodityInfo.profitRatio,
        100
      );
    },
    showBatch2(title) {
    ...
    api_order_listGoods_delete(e, deleteFlag) {
      if (this.batchTitle == this.$t("批量删除") || deleteFlag) {
        ...
        api_order_listGoods_delete_post(form)
          .then((e) => {
            console.log(e);
      ...
      if (this.batchTitle == this.$t("批量上架")) {
        ...
        api_goods_shelfBatch_post(form)
          .then((e) => {
            console.log(e);
            this.delVisible = false;
            this.batchVisible = false;
