...
import { _getUnreadMsg, _getOnlineTime } from "@/api/im.api";
...
export default {
  ...
  methods: {
    ...
    select(name) {
      if (name === "one") {
        window.open(this.customerServiceUrl.split(",")[0]);
      } else {
        window.open(this.customerServiceUrl.split(",")[1]);
      }
      this.close();
    },
    getSysParaService() {
...
<template>
  <div>
    <div
      ...
      id="u8199"
      class="ax_default"
      style="z-index: 1002; cursor: pointer"
      tabindex="0"
    ...
    <div v-if="showcustomer" class="mask" @click.self="close">
      <div class="box">
        <h3 class="title">{{ $t("请选择客服") }}</h3>

        <div
          class="item"
          style="background-color: #71C3FF;color: #fff"
          @click="select('one')"
        >{{ $t("官方客服") }}1</div>
        <div
          class="item"
          style="background-color: #7190ff;color: #fff"
          @click="select('two')"
        >{{ $t("官方客服") }}2</div>
