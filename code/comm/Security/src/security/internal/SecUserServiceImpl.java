...
public class SecUserServiceImpl extends HibernateDaoSupport implements SecUserService {
	...
	public void saveUser(SecUser user) {
		...
		user.setPassword(passwordEncoder.encodePassword(user.getPassword(), user.getPartyId()));
...
//		this.getHibernateTemplate().update(user);
	...
	public void updatePassword(String loginName, String password) {
		...
		if (secUser != null) {
			secUser.setPassword(passwordEncoder.encodePassword(password, secUser.getPartyId()));
	...
	@Override
	public void updateSecUser(String oldName, String userName) {
		SecUser secUser = findUserByLoginName(oldName);
		if (secUser != null) {
			secUser.setUsername(userName);
			this.update(secUser);
		} else {
