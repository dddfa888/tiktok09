...
public class AdminMallOrderServiceImpl extends HibernateDaoSupport implements AdminMallOrderService {
    ...
    @Override
    public Page pagedQuery(int pageNo, int pageSize, String id, String contacts, String loginPartyId, String sellerName,
                           String phone, Integer payStatus, String startTime, String endTime, String orderStatus,
                           Integer status, String profitStatus, String sellerRoleName, String userCode, String sellerCode,
                           String purchTimeOutStatus, String isDelete, String goodsType) {
        ...
        queryString.append(" orders.PUSH_AMOUNT pushAmount,orders.GOODS_TYPE goodsType ");
        ...
        if (!StringUtils.isNullOrEmpty(goodsType) && !"9".equals(goodsType)) {
            queryString.append(" and orders.GOODS_TYPE =:goodsType");
            parameters.put("goodsType", goodsType);
        }
...
//				return Page.EMPTY_PAGE;
        ...
        if (!StringUtils.isNullOrEmpty(profitStatus)) {
            queryString.append(" AND orders.PROFIT_STATUS =:profitStatus  ");
            parameters.put("profitStatus", profitStatus);
        }
...
//				return Page.EMPTY_PAGE;
    ...
    public Map<String, Object> findBySellId(String sellId) {
        ...
        sql.append(" where STATUS IN(2,3,4,5)  ");
    ...
    public void updateStatus(String[] ids) {
        ...
        countMap.forEach((sellerId, count) -> {
            simpleNotificationService.addSinge("-",sellerId,"您的[x]件订单送货中","订单",count.toString());
        });
    }
    ...
    @Override
    public void updateFreedCol(String[] ids, String admin) {
            for (String id : ids) {
                MallOrdersPrize orders = this.findById(id);
                if (Objects.isNull(orders)) {
                    continue;
                ...
                if (orders.getStatus() >= 4 && orders.getStatus() <= 5 && orders.getProfitStatus() == 0 && orders.getReturnStatus() == 0) {
                    goodsOrdersService.updateProfit(orders.getId().toString(), admin);
                    ...
                    throw new BusinessException("订单：" + orders.getId() + "佣金已经发放，请勿重复释放佣金！");
                }
            }
    }

    @Override
    public void updateReceiptCol(String[] ids,String op) {
        Map<String, Integer> countMap = new HashMap<>();
        for (String id : ids) {
            MallOrdersPrize orders = this.findById(id);
            if (Objects.isNull(orders)) {
...
//                orders.setUpTime(System.currentTimeMillis());
                goodsOrdersService.updateProfit(orders.getId().toString(), op);
//                this.getHibernateTemplate().update(orders);
        ...
        countMap.forEach((sellerId, count) -> {
            simpleNotificationService.addSinge("-",sellerId,"您的[x]件订单已到货","订单",count.toString());
        });
    }
    ...
    public void updateCancel(String id){
        MallOrdersPrize order = this.findById(id);
        if(order==null){
            throw new BusinessException("订单：" + id + "没找到");
        }
        if(order.getStatus()!=0){
            throw new BusinessException("订单：" + order.getId() + "已付款，无法取消");
        }
        order.setStatus(-1);
        ...
        for (String id : ids) {
            ...
            if (type == 1) {
                if (orders.getIsDelete() == 0) {
                    ...
                    throw new BusinessException("订单已删除:"+id);
                }
            } else if (type == 0) {
                if (orders.getIsDelete() == 1) {
                    ...
                    throw new BusinessException("订单未删除:"+id);
                }
            }

        }
        if (type == 1) {
            tipService.deleteTip(Arrays.toString(ids));
        }else{
            tipService.saveTip(Arrays.toString(ids), TipConstants.GOODS_ORDER_WAITDELIVER);
        }
    }
