...
public class GoodsOrdersServiceImpl extends HibernateDaoSupport implements GoodsOrdersService {
    ...
    public void setSimpleNotificationService(SimpleNotificationService simpleNotificationService) {
    ...
    @Override
    public void updateOrderRebate(String orderId) {
        String lockKey = RedisKeys.ORDER_LOCK + orderId;// 生成订单锁的 Redis 键
        if (!redisHandler.lock(lockKey, 20)) {// 尝试获取 Redis 锁，如果获取失败则直接返回，避免重复处理
            return;
        }
        MallOrdersPrize ordersPrize = this.getHibernateTemplate().get(MallOrdersPrize.class, orderId);// 获取订单
        if (Objects.isNull(ordersPrize)) {// 如果订单不存在，则直接返回
            return;
        }
        int status = ordersPrize.getStatus();// 获取订单的状态和利润状态
        int profitStatus = ordersPrize.getProfitStatus();

        try {
            if ((status == 4 || status == 5) && (profitStatus == 0 || profitStatus == 2)) {// 如果订单状态为 4 或 5 且利润状态为 0，则进行佣金结算
                log.info("订单[{}][status:{}][profit-status:{}]释放佣金!", orderId, status, profitStatus);
                String sellerId = ordersPrize.getSellerId();// 获取卖家 ID
                // 更新卖家余额
                this.updateWallet(orderId, sellerId, Constants.MONEYLOG_CONTNET_ORDER_INCOME, new BigDecimal(ordersPrize.getSystemPrice()), new BigDecimal(ordersPrize.getProfit()));
                ordersPrize.setProfitStatus(1);// 设置订单为已结算状态
                this.getHibernateTemplate().merge(ordersPrize);
                log.info("订单结算成功: {}, 更新卖家余额: {}", orderId, sellerId);
                // 更新卖家余额到 Redis 缓存
                this.redisHandler.setSync(RedisKeys.WALLET_PARTY_ID + sellerId, getWalletByPartyId(sellerId));
            }
        } catch (Exception e) {
    ...
    public Wallet getWalletByPartyId(String partyId) {
        String sql = "SELECT * FROM T_WALLET WHERE PARTY_ID = :partyId";
        MapSqlParameterSource parameter = new MapSqlParameterSource();// 创建查询参数
        ...
        return parameterJdbcTemplate.queryForObject(sql, parameter, (rs, rowNum) -> {
...
//        return orders;
    ...
    private MallOrdersPrize createOneOrder(Party party, MallAddress address, String sellerId,
        ...
        for (String googsId : goodsCountMap.keySet()) {
            SellerGoods sellerGoods = sellerGoodsMap.get(googsId);
            SystemGoods systemGoods = sellerGoods.getSystemGoods();
            String key = MallRedisKeys.TYPE_LANG + "cn:" + systemGoods.getCategoryId();
            String str = redisHandler.getString(key);
            if (i == 0 && !StringUtils.isEmptyString(str)) {
                i = 1;
                CategoryLang pLang = JSONArray.parseObject(str, CategoryLang.class);
                if (pLang.getName() != null && Arrays.asList("点卡", "话费", "积分", "虚拟商品").contains(pLang.getName())) {
                    goodsType = 1;
                }
            }
            // 同一个goods对根据skuId拆单
...
//              updateSellerGoods(sellerGoods);
        ...
        orders.setFees(BigDecimal.valueOf(fees).setScale(2, RoundingMode.DOWN).doubleValue());
        orders.setTax(BigDecimal.valueOf(tax).setScale(2, RoundingMode.DOWN).doubleValue());
        orders.setPrizeOriginal(BigDecimal.valueOf(prizeOriginal).setScale(2, RoundingMode.DOWN).doubleValue());
        orders.setPrizeReal(BigDecimal.valueOf(goodsReal).setScale(2, RoundingMode.DOWN).doubleValue());
        orders.setSystemPrice(BigDecimal.valueOf(systemPrice).setScale(2, RoundingMode.DOWN).doubleValue());
        orders.setGoodsCount(goodsCount);
        orders.setProfitStatus(0);

        // 改成用 BigDecimal 减法，保留两位小数
        BigDecimal real = BigDecimal.valueOf(goodsReal).setScale(2, RoundingMode.DOWN);
        BigDecimal sys = BigDecimal.valueOf(systemPrice).setScale(2, RoundingMode.DOWN);
        BigDecimal profit = real.subtract(sys).setScale(2, RoundingMode.DOWN);
        orders.setProfit(profit.doubleValue());
    ...
    public JSONArray saveGoodsBuy(String partyId, String uuid, String num) {
        ...
        for (String sellerId : orderMap.keySet()) {
            ...
            simpleNotificationService.addNone(partyId,sellerId,"订单","有用户下单请注意查收");
            orders.setSellerName(seller.getName());
            orders.setSellerDiscount(sellerDiscount);
            // orders.setStatus(0);
            orders.setStatus(1);
...
//        }
        ...
        try {
            ...
            double amount_before = updateWallet(wallet, BigDecimal.valueOf(prize));
            ...
            if (wallet.getFrozenState() == 0) {
                moneyLog.setAmount_after(wallet.getMoney() + wallet.getFrozenAmount());
                moneyLog.setFreeze(0);
            } else {// 处于冻结状态时
                moneyLog.setAmount_after(wallet.getMoneyAfterFrozen() + wallet.getFrozenAmount());
            ...
            for (MallOrdersPrize order : list) {
                ...
                countMap.put(sellerId, countMap.getOrDefault(sellerId, 0) + 1);
            ...
            countMap.forEach((sellerId, count) -> {
                simpleNotificationService.addNone(partyId,sellerId,"订单","有用户下单请注意查收");
            });
    ...
    public static Double updateWallet(Wallet wallet,BigDecimal money){
        if(wallet==null){
            throw new BusinessException("余额不足");
        }
        BigDecimal balance;
        if (wallet.getFrozenState() == 0) {
            balance = BigDecimal.valueOf(wallet.getMoney());
        } else {// 处于冻结状态时
            balance = BigDecimal.valueOf(wallet.getMoneyAfterFrozen());
        }
        BigDecimal frozenAmount = BigDecimal.valueOf(wallet.getFrozenAmount());// 冻结资金
        BigDecimal amount_before=balance.add(frozenAmount);

        if (amount_before.compareTo(money) < 0) {
            throw new BusinessException("余额不足");
        }
        BigDecimal amount = balance.subtract(money).setScale(2,RoundingMode.HALF_UP);
        if (amount.compareTo(BigDecimal.ZERO)<0) {// 余额不足扣除冻结金额
            amount=BigDecimal.ZERO;
            BigDecimal subtract = frozenAmount.subtract(money.subtract(balance));
            wallet.setFrozenAmount(subtract.setScale(2, RoundingMode.HALF_UP).doubleValue());
        }

        if (wallet.getFrozenState() == 0) {
            wallet.setMoney(amount.doubleValue());
        } else {// 处于冻结状态时
            wallet.setMoneyAfterFrozen(amount.doubleValue());
        }
        return amount_before.doubleValue();
    }
...
//            simpleNotificationService.addSinge(partyId,order.getSellerId(),"您的[x]件订单已到货","订单","1");
//        } catch (BusinessException e) {
//            logger.error("取消失败", e);
//            throw new BusinessException(e.getMessage());
//        }
...
//                2023-10-11  新增退款驳回不影响原始流程
    ...
    public MallPageInfo listMallOrdersPrize(String partyId, Integer status, int pageNum, int pageSize) {
        ...
        if (status != null) {
            if (status == 1) {
                query.add(Restrictions.in("status", Arrays.asList(1, 2)));
            } else {
                query.add(Property.forName("status").eq(status));
            }
        }else{
            query.add(Property.forName("status").gt(0));
        }
        query.addOrder(Order.desc("createTime"));
...
//            如果有折扣，按照采购折扣价格计算
            if (Double.compare(order.getSellerDiscount(), 0D) != 0) {
                prize = Arith.mul(prize, 1 - order.getSellerDiscount(), 2);
            }
            Double push = prize;
//            Wallet wallet = walletService.saveWalletByPartyId(partyId); 这里经常出现缓存不一致的问题，改为直接查库
            ...
            moneyLog.setCategory(Constants.MONEYLOG_CATEGORY_COIN);
            moneyLog.setAmount_before(amount_before);
            moneyLog.setAmount(Arith.sub(0, push));
            if (wallet.getFrozenState() == 0) {
                moneyLog.setAmount_after(wallet.getMoney() + wallet.getFrozenAmount());
                moneyLog.setFreeze(0);
            } else {// 处于冻结状态时
...
//            redisHandler.remove(lockKey);
    ...
    public Integer updateAutoReceipt() {
        ...
        for (String orderId : orderList) {
            MallOrdersPrize mallOrdersPrize = getMallOrdersPrize(orderId);
            if (Objects.nonNull(mallOrdersPrize)) {
                String sellerId = mallOrdersPrize.getSellerId();
                countMap.put(sellerId, countMap.getOrDefault(sellerId, 0) + 1);
                this.saveOrderLog(mallOrdersPrize.getPartyId(), orderId, OrderStatusEnum.ORDER_SEND_CONFIRM, "订单" + orderId + "订单已签收");
            }
        }
        countMap.forEach((sellerId, count) -> {
            simpleNotificationService.addSinge("task",sellerId,"您的[x]件订单已到货","订单",count.toString());
        });
        return result;
    }
    ...
    public List<MallOrdersPrize> listAutoProfit() {
        ...
        query.add(Property.forName("profitStatus").eq(0));// 手动释放
    ...
    public void updateAutoProfit(String orderId) {
        ...
        try {
            if ((status == 4 || status == 5) && profitStatus == 0) {
                ...
                ordersPrize.setUpTime(System.currentTimeMillis());
                String sellerId = ordersPrize.getSellerId();
                // Map<Integer, String> parents = this.findParents(sellerId);
                BigDecimal profit = BigDecimal.valueOf(ordersPrize.getProfit());
                ...
                // }
                    ...
                    result.add(sellerId);
                    this.updateWallet(orderId, sellerId, Constants.MONEYLOG_CONTNET_ORDER_INCOME, new BigDecimal(String.valueOf(ordersPrize.getSystemPrice())), remain);
                    ordersPrize.setProfitStatus(1);
                    this.getHibernateTemplate().merge(ordersPrize);
            ...
            redisHandler.remove(lockKey);  // 释放订单锁
        }
    }

    @Override
    public void updateProfit(String orderId,String admin) {
        ...
        try {
            if ((status ==3||status == 4 || status == 5) && profitStatus == 0) {
                log.info("订单[{}][status:{}][profit-status:{}]释放佣金! 操作人:{}", orderId, status, profitStatus,admin);
                ...
                result.add(sellerId);
                this.updateWalletWithLock(orderId, sellerId, Constants.MONEYLOG_CONTNET_ORDER_INCOME,
                        new BigDecimal(String.valueOf(ordersPrize.getSystemPrice())), remain);
                this.getHibernateTemplate().merge(ordersPrize);
    ...
    public void updateAutoIncreaseViewCount() {
        ...
        for (Seller seller : sellers) {
            ...
            if (combo != null) {// 直通车流量
                ...
                if (CollectionUtil.isNotEmpty(sellerComboGoodsIds)) {
                    log.info("店铺id:{} 直通车流量:{} 分配商品数: {}", sellerId, totalCombollViews, sellerComboGoodsIds.size());
                    addViewNum(goodsViewAddNum, totalCombollViews, sellerComboGoodsIds);
                }

            }
            // 基础流量
            if (Objects.equals(1, seller.getAutoValid())) {
                ...
                if (totaNormallViews > 0 && !allShelfGoodsId.isEmpty()) {
                    log.info("店铺id:{} 基础流量:{} 分配商品数: {}", sellerId, totaNormallViews, allShelfGoodsId.size());
    ...
    private void addSellerView(Seller seller, List<String> allShelfGoodsId, Map<String, Long> goodsViewAddNum) {
        ...
        if (StringUtils.isNotEmpty(mallLevel)) {
            ...
            if (CollectionUtils.isNotEmpty(mallLevels)) {
                ...
                log.info("店铺id:{} 等级流量:{} 分配商品数: {}", seller.getId(), totalViews, allShelfGoodsId.size());
                if (totalViews != 0) {
...
//        return baseTraffic * (autoStart + (int) (Math.random() * (autoEnd - autoStart + 1)));
    ...
    private void addViewNum(Map<String, Long> goodsViewAddNum, long totalCombollViews, List<String> goodsIds) {
        if (totalCombollViews == 0 || goodsIds.isEmpty()) {
            return;
        }
    ...
    public Map<String, Object> selectNoPushNum(String sellerId) {
        ...
        StringBuffer countSql = new StringBuffer("SELECT count(1) as noPushNum  from T_MALL_ORDERS_PRIZE  " +
                "where `STATUS` = 1 and IS_DELETE=0 and RETURN_STATUS=0 ");
     ...
     */
    @Override
    public Integer updateVirtualOrder() {
        // 查询采购时间是24小时之前订单
        Date purchTime = new Date(System.currentTimeMillis() - 24 * 60 * 60 * 1000);
        List<MallOrdersPrize> orders = this.getHibernateTemplate().execute(session ->
                session.createQuery("FROM MallOrdersPrize WHERE status in (2,3) and purchTime <=:purchTime " +
                                " and profitStatus=0 and goodsType=1 and returnStatus=0", MallOrdersPrize.class)
                        .setParameter("purchTime", purchTime)
                        .setMaxResults(100)
                        .list());
        ...
        Map<String, Integer> countMap = new HashMap<>();
        for (MallOrdersPrize order : orders) {
            String sellerId = order.getSellerId();
            countMap.put(sellerId, countMap.getOrDefault(sellerId, 0) + 1);
            Double mall_autoProfit = sysparaService.find("mall_autoProfit").getDouble();
            order.setUpTime(System.currentTimeMillis() - (long) (60 * 60 * 1000 * mall_autoProfit) - 1000);
...
//    }
    ...
    private void updateWallet(String orderId, String partyId, String type, BigDecimal principal, BigDecimal amount) {
        ...
        BigDecimal currentAmount = BigDecimal.valueOf(wallet.getMoney());
        BigDecimal afterAmount = currentAmount.add(totalAmount).setScale(2, RoundingMode.DOWN);
        BigDecimal rebateAmount = BigDecimal.valueOf(wallet.getRebate()).add(amount).setScale(2, RoundingMode.DOWN);
        wallet.setMoney(afterAmount.doubleValue());
        wallet.setRebate(rebateAmount.doubleValue());
        ...
        log.setAmount_before(currentAmount.setScale(2, RoundingMode.DOWN).doubleValue());
    ...
    public void updateWalletWithLock(String orderId, String partyId, String type, BigDecimal principal, BigDecimal amount) {
        Session session = getHibernateTemplate().getSessionFactory().getCurrentSession();
        Wallet wallet = null;

        try {
            // 1. 行锁
            wallet = (Wallet) session.createQuery("from Wallet where partyId = :partyId")
    ...
    @Override
    public List<MallOrdersPrize> listRealAutoOrderInfo() {
        List<Integer> returnStatusList = Arrays.asList(0, 3);
        DetachedCriteria query = DetachedCriteria.forClass(MallOrdersPrize.class);
        // 已采购
        query.add(Property.forName("purchStatus").eq(1));
        // 已发货
        query.add(Property.forName("status").eq(3));
        // 未退款/退款失败
        query.add(Property.forName("returnStatus").in(returnStatusList));
        // 真实商品
        query.add(Property.forName("goodsType").eq(0));
        List<MallOrdersPrize> results = (List<MallOrdersPrize>) getHibernateTemplate().findByCriteria(query, 0, 50);
        return results;
    }
     ...
     */
    @Override
    public double getLateFee(String partyId) {
        return this.getHibernateTemplate().execute(session ->
                (Double) session.createNativeQuery(
                                "SELECT IFNULL(ROUND(SUM(PRIZE_REAL), 2), 0) " +
                                        "FROM t_mall_orders_prize " +
                                        "WHERE SELLER_ID = :sellerId " +
                                        "AND STATUS in ( 2,3) "
                        )
                        .setParameter("sellerId", partyId)
                        .getSingleResult()
        );
    }
     ...
     */
    @Override
    public double getTotalRebate(String sellerId) {
        return this.getHibernateTemplate().execute(session ->
                (Double) session.createNativeQuery(
                                "SELECT IFNULL(ROUND(SUM(PRIZE_REAL), 2), 0) " +
                                        "FROM t_mall_orders_prize " +
