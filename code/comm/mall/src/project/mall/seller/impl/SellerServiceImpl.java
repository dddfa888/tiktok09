...
//            getHibernateTemplate().clear();
    ...
    public Map<String, Object> findBySellId(String sellId) {
        ...
        if (StringUtils.isEmptyString(sellId)) {
            sql.append(" IFNULL(SUM(sellers.FAKE), 0) AS FAKE, ");
            sql.append(" IFNULL(SUM(sellers.CREDIT_SCORE), 0) AS CREDIT_SCORE, ");
            sql.append(" IFNULL(SUM(sellers.REALS), 0) AS REALS, ");
            sql.append(" 0 AS SCORE ");
            sql.append(" FROM T_MALL_SELLER sellers ");
        } else {
            sql.append(" IFNULL(sellers.FAKE, 0) AS FAKE, ");
            sql.append(" IFNULL(sellers.CREDIT_SCORE, 0) AS CREDIT_SCORE, ");
            sql.append(" IFNULL(sellers.REALS, 0) AS REALS, ");
            sql.append(" IFNULL(sellers.SCORE, 0) AS SCORE ");
            sql.append(" FROM T_MALL_SELLER sellers ");
            sql.append(" WHERE sellers.UUID = ? ");
        }

        NativeQuery<Object[]> nativeQuery = this.getHibernateTemplate()
                .getSessionFactory()
                .getCurrentSession()
        ...
        sumData.put("focusCount", results[0]);      // 对应 FAKE
        sumData.put("creditScore", results[1]);     // 对应 CREDIT_SCORE
        sumData.put("focusCountReals", results[2]); // 对应 REALS
        sumData.put("score", results[3]);           // 现在无论如何，results[3] 都是安全的
    ...
    public Map<String, Object> loadReportTotalSales(String sellerId, String startTime, String endTime) {
        ...
        sql.append(" where  STATUS IN(1,2,3,4,5)  AND RETURN_STATUS =0 ");
    ...
    public Map<String, Object> loadReportTotalProfit(String sellerId, String startTime, String endTime) {
        ...
        StringBuffer sql = new StringBuffer("SELECT cast(SUM(IFNULL(PROFIT,0)) AS DECIMAL (19, 3)) AS totalProfit " +
                "  FROM T_MALL_ORDERS_PRIZE WHERE STATUS IN  (2,3,4,5)  " );
...
//        moneyLogService.save(moneyLog);
    ...
    @Override
    public Long payOrderNum(String sellerId, String startTime, String endTime) {
        StringBuffer sql = new StringBuffer(" select count(1) from T_MALL_ORDERS_PRIZE where STATUS=1 and PAY_STATUS=1 " );
        ...
        return Long.valueOf(results+"");
    }

    @Override
    public Long sendOutOrderNum(String sellerId, String startTime, String endTime) {
        StringBuffer sql = new StringBuffer(" select count(1) from T_MALL_ORDERS_PRIZE where STATUS=3 and PAY_STATUS=1 " );
    ...
    @Override
    public Map<String, Object> countStatus(String sellerId, String startTime, String endTime) {
        Map<String, Object> queryParams = new HashMap<>();

        NamedParameterJdbcTemplate namedParameterJdbcTemplate1 = new NamedParameterJdbcTemplate(jdbcTemplate);
        StringBuffer sql=new StringBuffer();
        sql.append("SELECT " +
                "  COALESCE(SUM(CASE WHEN STATUS = 1 THEN 1 ELSE 0 END), 0) AS status1," +
                "  COALESCE(SUM(CASE WHEN STATUS = 2 THEN 1 ELSE 0 END), 0) AS status2," +
                "  COALESCE(SUM(CASE WHEN STATUS = 3 THEN 1 ELSE 0 END), 0) AS status3," +
                "  COALESCE(SUM(CASE WHEN STATUS = 4 THEN 1 ELSE 0 END), 0) AS status4," +
                "  COALESCE(SUM(CASE WHEN STATUS = 5 THEN 1 ELSE 0 END), 0) AS status5 " +
//                "  COALESCE(SUM(CASE WHEN STATUS = 6 THEN 1 ELSE 0 END), 0) AS status6 " +
                "  FROM T_MALL_ORDERS_PRIZE WHERE 1 = 1 ");
        if (StringUtils.isNotEmpty(startTime) && StringUtils.isNotEmpty(endTime)) {
            queryParams.put("start", startTime);
            queryParams.put("end", endTime);
            sql.append(" AND CREATE_TIME BETWEEN :start AND :end ");
        }

        if (StringUtils.isNotEmpty(sellerId)) {
            queryParams.put("id", sellerId);
            sql.append(" AND SELLER_id = :id ");
        }
        List<Map<String, Object>> lists = namedParameterJdbcTemplate1
                .queryForList(sql.toString(), queryParams);
        return lists.get(0);
    }
