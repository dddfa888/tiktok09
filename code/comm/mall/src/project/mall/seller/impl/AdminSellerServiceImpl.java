...
public class AdminSellerServiceImpl extends HibernateDaoSupport implements AdminSellerService {
    ...
    public Page pagedQuery(int pageNo, int pageSize, String name_para, String loginPartyId, String sellerId, String sellerName, String startTime,
        ...
        queryString.append(" s.UUID sellerId,s.STATUS status, s.CREATE_TIME createTime, s.NAME sellerName, s.AVATAR avatar, s.SHOP_PHONE shopPhone, s.MALL_LEVEL level, ");
        queryString.append(" wallet.MONEY money, wallet.FROZEN_AMOUNT moneyFrozen,wallet.FROZEN_STATE frozenState," +
                " s.REC_TIME recTime, s.FREEZE freeze,wallet.REBATE rebate, ");
        ...
        queryString.append(" party_parent.USERNAME username_parent, kyc.REMARK remark, party.REMARKS remarks,s.SCORE score,s.TAG, ");
...
//    this.pageNo, this.pageSize, userName, userCode,sellerName,flag,startTime, endTime
    ...
    public void updateScore(String id, String score) {
        Double v = Double.valueOf(score);
        this.getHibernateTemplate().execute(session ->
                session.createQuery("UPDATE Seller SET score = :score WHERE id = :id")
                        .setParameter("score", v)
                        .setParameter("id", id)
                        .executeUpdate());
    }

    @Override
    public void updateSeller(String id, String sellerName, String remarks, int freeze, int black, int status) {
