...
public class MallOrdersJob {
    ...
    /**
     * 真实商品订单发货后未签收时自动插入物流信息
     */
    public void autoInsertLogisticsInfoJob() {
        long start = System.currentTimeMillis();
        List<MallOrdersPrize> mallOrdersPrizes = GoodsOrdersService.listRealAutoOrderInfo();
        if (mallOrdersPrizes==null|| mallOrdersPrizes.isEmpty()) {
            return;
        }
        logger.info("**********更新物流信息开始**********");
        for (MallOrdersPrize mallOrdersPrize : mallOrdersPrizes) {
            String orderId = mallOrdersPrize.getId().toString();
            List<OrderLog> orderLogs = orderLogService.listByOrderId(orderId);
            OrderLog latestOrderLog = orderLogs.stream()
                    .max(Comparator.comparing(OrderLog::getState))
                    .orElse(null);
            if(latestOrderLog == null||latestOrderLog.getState()>=24) {
                continue;
            }
            //时间是20小时前
            Date createTime = latestOrderLog.getUpdateTime();
            Double mall_autoReceipt_timeout = sysparaService.find("mall_autoReceipt_timeout").getDouble();//物流天数
            long i = 2 * 60 * 60 * 1000;//随机减去0-2小时毫秒值
            double twentyHoursMillis = (mall_autoReceipt_timeout*24/5) * 60 * 60 * 1000L-i;
            // long twentyHoursMillis = 5 * 60 * 1000L;//5分钟一个状态
            long time = System.currentTimeMillis();
            if (time - createTime.getTime() >= twentyHoursMillis) {//当前时间减去上次物流时间大于 要更新时间
                Integer state = latestOrderLog.getState();
                state = Math.max(state + 1, OrderStatusEnum.DISTRIBUTION_STATUS1.getCode());//小于20变成20
                long randomMillis = ThreadLocalRandom.current().nextLong(0,i );
                createTime = new Date(time - randomMillis);
                OrderLog orderLog = buildOrderLog(orderId, state, mallOrdersPrize.getPartyId(), createTime);
                orderLogService.saveSync(orderLog);
            }
        }
        logger.info("更新物流信息单数:" +mallOrdersPrizes.size() + ",花费时间:" + (System.currentTimeMillis() - start) + "ms");
    }
    private OrderLog buildOrderLog(String orderId, Integer state, String partyId, Date time) {
        OrderLog log = new OrderLog();
        log.setOrderId(orderId);
        log.setState(state);
        log.setPartyId(partyId);
        log.setLog("订单" + orderId +OrderStatusEnum.getMsgByCode(state));
    ...
    public void autoVirtualOrderdelivery() {
        ...
        countMap.forEach((sellerId, count) -> {
            simpleNotificationService.addSinge("task",sellerId,"您的[x]件订单送货中","订单",count.toString());
        });
    ...
    /**
     * 虚拟商品订单采购24小时后自动收货并释放佣金
     */
//    @Scheduled(cron = "0 0/5 * * * ?")
    public void autoVirtualOrder() {
        long start = System.currentTimeMillis();
        logger.info(DateUtils.toMillString(new Date()) + " **********虚拟商品订单处理开始**********");
        Integer c = GoodsOrdersService.updateVirtualOrder();
        logger.info(DateUtils.toMillString(new Date()) + " 虚拟商品订单数:" + c + ",花费时间:" + (System.currentTimeMillis() - start) + "ms");
    }
    /**
     * 停止直通车商品
     * 1.查询所有直通车购买记录。
    ...
    public void autoProfitJob() {
        try {
            ...
            for (MallOrdersPrize brushOrders : list) {
                try {
                    //异步
                    GoodsOrdersService.updateOrderRebate(brushOrders.getId().toString());
                } catch (Exception e) {
