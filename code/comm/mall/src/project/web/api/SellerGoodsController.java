...
//            List<KeepGoods> keepGoods = null;
//            if (null != partyId) {
//                try {
//                String pkey =  MallRedisKeys.MALL_GOODS_COLLECTION+lang+"-"+partyId;
//                Object o = redisHandler.get(pkey);
//                ObjectMapper objectMapper = new ObjectMapper();
//                if (ObjectUtils.isNotEmpty(o) && o !="null"){
//                        keepGoods = objectMapper.readValue(o.toString(), new TypeReference<List<KeepGoods>>(){});
//                }else {
//                    keepGoods = keepGoodsService.queryIsKeeps(partyId);
//                    redisHandler.setAsyn(pkey, objectMapper.writeValueAsString(keepGoods));
//                }
...
//                if (CollectionUtil.isEmpty(keepGoods)) {
////                    goodsVo.setIsKeep(keepGoodsService.queryIsKeep(pl.getId().toString(), partyId));
//                    boolean b = keepGoods.stream().anyMatch(goods -> goods.getSellerGoodsId().equals(pl.getId()));
//                    goodsVo.setIsKeep(b?1:0);
//                }
    ...
    public Object searchKeyword(HttpServletRequest request) {
        ...
        for (SellerVo sellerVo : sellerVoList) {
            Map<String, Object> bySellId = sellerService.findBySellId(sellerVo.getId() + "");
            Double s=(Double)bySellId.get("score");//设置分数优先显示
            Double goodEvaluationRatio;
            if(s==0){
                goodEvaluationRatio = evaluationStatMap.get(sellerVo.getId());
            }else{
                goodEvaluationRatio = BigDecimal.valueOf(s / 5).setScale(2, RoundingMode.HALF_UP).doubleValue();
            }
...
//        String lang = this.getLanguage(request);
        ...
        goodsVo.setViewsNum(sellerGoodsService.getViewNums(sellerGoods.getId().toString())+sellerGoods.getViewsNum());
        ...
        imgDes = imgDes.replaceAll("(?i)<img[^>]*>", "");
...
//        goodsVo.setFreightAmount(sellerGoods.getSystemGoods().getFreightAmount());
    ...
    public Object search(HttpServletRequest request) {
        ...
        List<String> sellerGoodsId = list.stream().map(s -> s.getId().toString()).collect(Collectors.toList());
        Map<String, Long> viewNums = sellerGoodsService.getViewNums(sellerGoodsId);
        List<KeepGoods> keepGoods = keepGoodsService.queryIsKeeps(partyId);
        for (SellerGoods pl : list) {
            String js = redisHandler.getString(MallRedisKeys.MALL_GOODS_LANG + lang + ":" + pl.getGoodsId());
            if (kernel.util.StringUtils.isEmptyString(js)) {
                continue;
            ...
            goodsVo.setShowWeight1(pl.getShowWeight1());
            goodsVo.setShowWeight2(pl.getShowWeight2());
            if (null != partyId && keepGoods != null && !keepGoods.isEmpty()) {
                boolean b = keepGoods.stream().anyMatch(goods -> goods.getId().equals(pl.getId()));
                goodsVo.setIsKeep(b?1:0);
//                goodsVo.setIsKeep(keepGoodsService.queryIsKeep(pl.getId().toString(), partyId));
            }
            Long time = pl.getRecTime();
