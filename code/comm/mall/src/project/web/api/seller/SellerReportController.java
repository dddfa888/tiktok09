...
public class SellerReportController extends BaseAction {
    ...
    public Object head(HttpServletRequest request) {
        ...
        if (StringUtils.isEmptyString(sellerId)) {
            ...
            o.put("willIncome", 0);      // 待到账金额
            o.put("totalSales", 0);      // 总销售额
            o.put("totalProfit", 0);     // 总利润
            o.put("orderNum", 0);        // 总订单
            o.put("orderReturns", 0);    // 退货订单数
            o.put("orderCancel", 0);     // 取消订单
            o.put("payOrder", 0);        // 已付款
            o.put("sendOutOrder", 0);    // 发货中
            o.put("unconfirmed", 22);    // 未确认
            o.put("arrivedGoods", 33);    // 已到货
        ...
        Map<String, Object> statusCount = sellerService.countStatus(sellerId, startTime, endTime);
        o.put("payOrder", statusCount.get("status2"));        // 已付款2
        o.put("sendOutOrder", statusCount.get("status3"));    // 发货中3
        o.put("unconfirmed", statusCount.get("status1"));    // 未确认1
        o.put("arrivedGoods", Integer.parseInt(statusCount.get("status4")+"")+Integer.parseInt(statusCount.get("status5")+""));    // 已到货45
        JSONObject object = new JSONObject();
        object.put("head", o);
        resultObject.setData(object);
        return resultObject;
