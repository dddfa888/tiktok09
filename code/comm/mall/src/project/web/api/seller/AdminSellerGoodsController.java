    ...
    public Object list(HttpServletRequest request) {
        ...
        if (CollectionUtil.isEmpty(list)) {
            ...
            for (SellerGoods pl : list) {
                ...
                String cname=cLang==null?"":cLang.getName();
                goodsVo.setCategoryName(cname);
...
//        object.put("evaluations", evaluationNum);
    ...
    public Object addOrUpdate(HttpServletRequest request) {
        ...
        Syspara syspara = sysparaService.findByDB("seller_max_goods_on_number");
        if (syspara != null) {
            long goodNum = sellerGoodsService.getOnSelfGoodsNumBySellerId(partyId); //上架商品数
            int maxGoodNum = Integer.parseInt(syspara.getValue()); //最大上架商品数
            if ((goodNum + goodsIdsArray.length) > maxGoodNum) {
                resultObject.setCode("1");
                resultObject.setMsg("大于店铺设置最大上架商品数");
                return resultObject;
            }
        }
...
//              2023-10-28 统一保留俩位小数，直接舍去
            ...
            int sold = mapValue2(systemGoods.getSystemPrice().intValue());
            int range = randomMinMax(221, 377);
            int views = sold * range;
            views /= 100;
            sellerGoods.setSoldNum(sold);
            sellerGoods.setViewsNum(views);
            seller.setFakeSoldNum(seller.getFakeSoldNum()==null?0:seller.getFakeSoldNum() + sold);
    ...
    int mapValue2(int input) {// 输入越大输出越小
        int num = 0;
        if (input < 100) {
            num = randomMinMax(4000, 5000);
        } else if (input < 1000) {
            num = randomMinMax(2000, 4000);
        } else if (input < 5000) {
            num = randomMinMax(500, 3000);
        } else if (input < 10000) {
            num = randomMinMax(300, 2000);
        } else if (input < 15000) {
            num = randomMinMax(100, 1000);
        } else {
            num = randomMinMax(50, 500);
        }
        return num;
    }
