...
//    protected CommonNotifyService commonNotifyService;
    ...
    public ModelAndView list(HttpServletRequest request) {
        ...
        try {
            ...
            for (int i = 0; i < list.size(); i++) {
                Map map = list.get(i);
                double totalRebate=goodsOrdersService.getTotalRebate((String) map.get("sellerId"));
    ...
    public ModelAndView updateSeller(HttpServletRequest request) {
        String pageNo = request.getParameter("pageNo");
        String id = request.getParameter("sellerId8");
        String sellerName = request.getParameter("sellerName8");
        String status = request.getParameter("status8");
        String freeze = request.getParameter("freeze8");
        String black = request.getParameter("black8");
        String remarks = request.getParameter("remarks8");
        ModelAndView model = new ModelAndView();
        model.addObject("pageNo",pageNo);
        try {
            ...
            if (!("0".equals(freeze) || "1".equals(freeze))) {
                throw new IllegalArgumentException("error");
            }
            if (!("0".equals(black) || "1".equals(black))) {
                throw new IllegalArgumentException("error");
            }
            VerifyUtils.validSellerNameError(sellerName);
            partyService.updateUserRemark(id,remarks);
            adminSellerService.updateSeller(id,sellerName,remarks,Integer.parseInt(freeze),
                    Integer.parseInt(black),Integer.parseInt(status));
        } catch (BusinessException e) {
            model.addObject("error", e.getMessage());
            model.setViewName("redirect:/" +  "mall/seller/list.action");
            return model;
...
//    }
    ...
    public Map LoginFree(HttpServletRequest request) {
        ...
        try {
            ...
            if(CommonNotifyManager.system.equals(UserRedisKeys.SYSTEM_WIN)){
                loginUrl = "http://localhost:9527/#/login?token=" + token;
            }else{
                loginUrl = AutoConfig.attribute("dm_url") + "/ww/#/login?token=" + token;
            }
     ...
     * 平台登录商家
     */
    @RequestMapping("/LoginFreeUser.action")
    public Map LoginFreeUser(HttpServletRequest request) {
        Map<String, Object> resultMap = new HashMap<String, Object>();
        String id = request.getParameter("id");
        try {
            String token = adminSellerService.getLoginFree(id,this.getUsername_login());
            String loginUrl;
            if(CommonNotifyManager.system.equals(UserRedisKeys.SYSTEM_WIN)){
                loginUrl = "http://localhost:8083/ww/#/login?token=" + token;
            }else{
                loginUrl = AutoConfig.attribute("dm_url") + "/ww/#/login?token=" + token;
            }
            resultMap.put("loginUrl",loginUrl);
...
//                sendInboxNotify(bizType, language, targetUserId, targetTopic, varDataMap);
    ...
    @PostMapping("/freezeSeller.action")
    public ModelAndView freezeSeller(HttpServletRequest request) {
        ModelAndView model = new ModelAndView();
        String sellerId = request.getParameter("seller_Id44");
        try {
           this.adminSellerService.updateFreeze(sellerId,1);
     ...
     * 解冻商家
     */
    @PostMapping("/unFreezeSeller.action")
    public ModelAndView unFreezeSeller(HttpServletRequest request) {
        ModelAndView model = new ModelAndView();
        String sellerId = request.getParameter("seller_Id45");
        try {
            this.adminSellerService.updateFreeze(sellerId,0);
...
//    }
     ...
     * 修改综合评分
     */
    @RequestMapping(value = "/update-score")
    public ModelAndView updateTag(HttpServletRequest request) {
        String pageNo = request.getParameter("pageNo7");
        String sellerId = request.getParameter("sellerId7");
        String s = request.getParameter("score7");
        ModelAndView model = new ModelAndView();
        model.addObject("pageNo", pageNo);
        if (s == null || !s.matches("^([0-4](\\.\\d{1})?|5(\\.0)?)$")) {
            model.addObject("error", "综合评分必须是 0 到 5 之间,且最多 1 位小数");
            model.setViewName("redirect:/" + "mall/seller/list.action");
            return model;
        }
        try {
            adminSellerService.updateScore(sellerId,s);
        } catch (BusinessException e) {
            model.addObject("error", e.getMessage());
            model.setViewName("redirect:/" + "mall/seller/list.action");
            return model;
     ...
     * 发送通知
     */
    @RequestMapping(value = "/send-tip")
    public ModelAndView sendTip(HttpServletRequest request) {
        String pageNo = request.getParameter("pageNo16");
        String sellerId = request.getParameter("sellerId16");
        String content = request.getParameter("content16");
        ModelAndView model = new ModelAndView();
        model.addObject("pageNo", pageNo);
        if (content == null || content.length()>100) {
            model.addObject("error", "通知长度不能超过100");
            model.setViewName("redirect:/" + "mall/seller/list.action");
            return model;
        }
        try {
            SecUser sec = this.secUserService.findUserByLoginName(this.getUsername_login());
            simpleNotificationService.addCustomize(sec.getId()+"",sellerId,content);
        } catch (BusinessException e) {
            model.addObject("error", e.getMessage());
            model.setViewName("redirect:/" + "mall/seller/list.action");
            return model;
