    ...
    public Page pagedQuery(int pageNo, int pageSize, String name_para, String status_para, String rolename_para,
        ...
        queryString.append(
                " kyc.UUID id,kyc.IDNUMBER idnumber,kyc.IDNAME idname,kyc.NAME name,seller.NAME sellerName,seller.AVATAR sellerImg,seller.SHOP_ADDRESS sellerAddress,"
                        + "kyc.IDIMG_1 idimg_1,kyc.IDIMG_2 idimg_2,kyc.SELLER_IMG idimg_3,kyc.APPLY_TIME apply_time,kyc.OPERATION_TIME operation_time, kyc.REMARK remarks,"
...
//		getHibernateTemplate().flush();
    ...
    @Override
    public Kyc getKycBySellerId(String sellerId) {
        List<Kyc> not = this.getHibernateTemplate().execute(session ->
                session.createQuery("FROM Kyc WHERE PARTY_ID = :sellerId", Kyc.class)
                        .setParameter("sellerId", sellerId)
                        .list());
        return CollectionUtils.isEmpty(not) ? null : not.get(0);
    }
