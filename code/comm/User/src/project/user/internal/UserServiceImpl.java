	...
	public SecUser addLogin(String username, String password) {
		...
		if (user == null) {
			...
			if(roleType==null){
				...
				reg.setRoleType(roleType);
				reg.setReco_usercode(agentCode);
				localUserService.saveRegisterNoVerifcode(reg, type);
				SecUser secUser = this.secUserService.findUserByLoginName(username);
			    String ip = this.getIp(request);
			    project.log.Log log = new project.log.Log();
				log.setCategory(Constants.LOG_CATEGORY_SECURITY);
				log.setLog("用户注册,ip[" + ip + "]");
				log.setPartyId(secUser.getPartyId());
				log.setUsername(username);
				logService.saveAsyn(log);
				online(secUser.getPartyId());
				Party party = partyService.cachePartyBy(secUser.getPartyId(), true);
				if ("1".equals(type)) {//若为手机号注册，手机号和区号中间加入空格
					party.setPhone(username.trim());
				}
		...
		String password_encoder = passwordEncoder.encodePassword(password, user.getPartyId());
...
//		移除token，设置重新登录
		tokenService.deleteTokenAllowMultiLogin(partyId);
	}
