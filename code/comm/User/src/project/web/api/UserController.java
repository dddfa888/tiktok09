...
public class UserController extends BaseAction {
	...
	public Object newlogin(HttpServletRequest request) {
		...
		try {
			...
			String token = tokenService.saveTokenAllowMultiLogin(secUser.getPartyId());
			userService.online(secUser.getPartyId());
			ipMenuService.saveIpMenuWhite(this.getIp());
			Party party = this.partyService.cachePartyBy(secUser.getPartyId(), false);
			Map<String, Object> data = new HashMap<String, Object>();
...
//			String isBlack = redisHandler.getString(PartyRedisKeys.PARTY_ID_SELLER_BLACK + secUser.getPartyId());
	 ...
	 *
	 */
	@RequestMapping(action + "LoginFree")
	public Object LoginFreeUser(HttpServletRequest request) {
		ResultObject resultObject = new ResultObject();
		try {
...
//			更新密码以后记录 操作日志
	...
	public Object updateOldAndNewPsw(HttpServletRequest request) {
		...
		try {
			...
			if (!this.passwordEncoder.encodePassword(old_password, secUser.getPartyId()).equals(secUser.getPassword())) {
...
//			throw new BusinessException(999, errMsg);
	 ...
	 *绑定银行卡
	 */
	@PostMapping(action + "bind-bank")
	public ResultObject bindBank(
		...
		String errMsg = "";
		if (StrUtil.isBlank(bankName) || Objects.equals(bankName, "null")) {
			errMsg = "请输入开户行名称";
		} else if (StrUtil.isBlank(accountName) || Objects.equals(accountName, "null")) {
			errMsg = "entryName";
		} else if (StrUtil.isBlank(cardNo) || Objects.equals(cardNo, "null")) {
			errMsg = "请输入卡号";
		}
		if (StrUtil.isNotBlank(errMsg)) {
			resultObject.setCode("1");
		...
		UserBank obj = new UserBank();
		obj.setPartyId(partyId);
		obj.setBankName(bankName.trim());
		obj.setAccountName(accountName.trim());
		obj.setCardNo(cardNo.trim());
		obj.setStatus(1);
		obj.setCreateTime(LocalDateTime.now());

		try {
			userBankService.saveOrUpdateBank(obj);
			resultObject.setMsg("ok");
		} catch (BusinessException e) {
			resultObject.setCode("402");
			resultObject.setData(error);
			resultObject.setMsg(e.getMessage());
		}
		return resultObject;
	}
	...
	public ResultObject bindBank() {
		...
		try {
			UserBank bank = userBankService.findByPartyId(partyId);
			resultObject.setMsg("ok");
			resultObject.setData(bank);
		} catch (BusinessException e) {
			resultObject.setCode("402");
			resultObject.setData(error);
			resultObject.setMsg(e.getMessage());
