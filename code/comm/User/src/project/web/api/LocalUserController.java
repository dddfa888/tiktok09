...
//		}
    ...
    public Object registerNoVerifcode(HttpServletRequest request) {
        ResultObject resultObject = new ResultObject();
//        String verifcode = request.getParameter("verifcode").replace(" ", "");
...
//				新邮箱必须未在EMAIL和USERNAME字段中使用过
     ...
     * 绑定手机-无确认
     */
    @RequestMapping(action + "bindPhone.action")
    public Object bindPhone(HttpServletRequest request) {
        String target = request.getParameter("target");
        String phone = request.getParameter("phone");
        String password = request.getParameter("password");
