...
//		this.getHibernateTemplate().flush();
	...
	@Override
	@Transactional
	public Map saveSucceeded(String order_no, String operator, String transfer_usdt, String success_amount,
							 double rechargeCommission,String remark,int type) {
...
//		wac.publishEvent(new RechargeSuccessEvent(this, info));
		...
		if(type!=1&& Constants.SECURITY_ROLE_MEMBER.equals(party.getRolename())){
...
//		}
		...
		result.put("pid",recharge.getPartyId().toString());
		return result;
	}
	/**
	 * 充值手动审核通过后续处理逻辑
	...
	@Override
	@Transactional
	public void saveSucceeded(String order_no) {
		Map result = new HashMap();
		...
		log.setPartyId(recharge.getPartyId());
		log.setCreateTime(date);
		log.setLog("yunhui支付。订单号[" + order_no+ "]。");
		logService.saveSync(log);
		walletLog.setAmount(amount);
		...
		walletService.update(wallet.getPartyId().toString(), amount, 0.0, 0.0);
		...
		log1.setCategory(Constants.LOG_CATEGORY_OPERATION);
		log1.setExtra(order_no);
		log1.setOperator("yunhui");
		log1.setUsername(secUser.getUsername());
		log1.setPartyId(recharge.getPartyId());
		log1.setCreateTime(new Date());
		log1.setLog("第三方yunhui支付。订单号[" + order_no + "]。");

		logService.saveSync(log1);
		tipService.deleteTip(recharge.getId().toString());
		debugLogger.info("-----> 充值订单第三方支付完成:{} 提交了相关的提示消息删除请求", recharge.getId());
	}
