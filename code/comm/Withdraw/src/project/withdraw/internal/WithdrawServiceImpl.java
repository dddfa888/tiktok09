...
//		KycHighLevel party_kycHighLevel = this.kycHighLevelService.get(withdraw.getPartyId().toString());
		...
		if ("bank".equals(channel)||"wx".equals(channel)||"zfb".equals(channel)) {//微信支付宝和银行卡用用一个
			withdraw_fee = Double.parseDouble(sysparaService.find("withdraw_other_channel_fee_part_bank").getValue());
		}else {
			withdraw_fee = Double.parseDouble(sysparaService.find("withdraw_fee").getValue());
		}
...
// 		}
		withdraw_fee = Arith.div(withdraw_fee, 100);
		double fee = Arith.mul(withdraw.getVolume(), withdraw_fee);
		...
		String upperChannel = channel.toUpperCase(); // 统一转大写
		if (upperChannel.matches(".*(USDT|USDC|BANK|WX|ZFB).*")) {
		...
		if (!channel.equalsIgnoreCase("wx")){
