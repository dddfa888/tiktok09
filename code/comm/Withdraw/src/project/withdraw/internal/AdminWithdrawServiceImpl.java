...
//					Arith.add(withdraw.getAmount(), withdraw.getAmount_fee()));
		...
		simpleNotificationService.addSinge(sec.getId()+"",withdraw.getPartyId()+"","您的提现失败,金额","提现",withdraw.getAmount()+"");
...
//		}
		...
		if (withdraw != null && withdraw.getSucceeded() == 0) {
			 ...
			 * 提现订单加入userdate
			 */
			Party party = partyService.getById(withdraw.getPartyId()+"");
			if (null == party) {
			...
			simpleNotificationService.addSinge(sec.getId()+"",withdraw.getPartyId()+"","您的提现成功,金额","提现",withdraw.getAmount()+"");
			SecUser SecUser = secUserService.findUserByPartyId(withdraw.getPartyId());
			Log log = new Log();
			log.setCategory(Constants.LOG_CATEGORY_OPERATION);
			log.setExtra(withdraw.getOrder_no());
