...
public class WithdrawController extends BaseAction {
    ...
    public Object getWithdrawLimitConfig() {
        ...
        try {
            ...
            Double withdrawAmountMin2 = sysparaService.find("withdraw_limit2").getDouble();
            Double withdrawAmountMax2 = sysparaService.find("withdraw_limit_max2").getDouble();

            Map<String, Object> data = new HashMap<String, Object>();
            data.put("withdrawAmountMin", withdrawAmountMin);
            data.put("withdrawAmountMax", withdrawAmountMax);
            data.put("withdrawAmountMin2", withdrawAmountMin2);
            data.put("withdrawAmountMax2", withdrawAmountMax2);
    ...
    public Object apply(HttpServletRequest request) throws IOException {
        ...
        // 银行卡提现模式，此处填写用户姓名
        String bankUserName = request.getParameter("bankUserName");//支付宝微信名字
        // 银行卡提现模式，此处填写银行卡号
        String bankCardNo = request.getParameter("bankCardNo");//支付宝微信号
        // 银行卡提现模式，此处填写路由号码
        ...
        // 银行卡提现模式，此处填写国际代码
        // String swiftCode = request.getParameter("swiftCode");//取消备注
        // 微信提现
        String qdCode = request.getParameter("qdCode");
        ...
        try {
            ...
            if (!Arrays.asList("bank", "wx", "zfb").contains(channel.toLowerCase())) {
...
//			}
            ...
            if (seller != null) {
                int freezeStatus = seller.getFreeze() != null ? seller.getFreeze() : 0;
                if (freezeStatus == 1) {
                    throw new BusinessException("店铺已冻结无法提现");
                }
            }
            ...
            Double withdrawalMin, withdrawalMax;
            if("zfb".equalsIgnoreCase(channel)){
                if(bankUserName==null||bankUserName.trim().isEmpty()){
                    throw new BusinessException("参数错误");
                }
            }
//            if (Arrays.asList("wx", "zfb").contains(channel.toLowerCase())) {
//                if(bankCardNo==null||bankCardNo.trim().isEmpty()){
//                    throw new BusinessException("参数错误");
//                }
//                withdrawalMin = sysparaService.find("withdraw_limit2").getDouble();
//                withdrawalMax = sysparaService.find("withdraw_limit_max2").getDouble();
//            } else {
//                withdrawalMin = sysparaService.find("withdraw_limit").getDouble();
//                withdrawalMax = sysparaService.find("withdraw_limit_max").getDouble();
//            }
//            if (usdtAmount < withdrawalMin) {
...
//            }
            ...
            //     partyService.updateUserRemark(partyId, swiftCode);
            // }
            ...
            if ("wx".equalsIgnoreCase(channel)) {
                if(qdCode==null||qdCode.isEmpty()){
                    throw new BusinessException("请上传微信收款码");
                }
                withdraw.setQdcode(qdCode);
            }
            // 保存
...
//			}
    ...
    public Object limit(HttpServletRequest request) throws IOException {
        ...
        try {
            ...
            }else if (channel.contains("WX")||channel.contains("ZFB")) {
                map.put("limit", this.sysparaService.find("withdraw_limit_2").getValue());
            }else {
