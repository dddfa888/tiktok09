...
public class NewOnlineChatController extends BaseAction {
	...
	public Object list(HttpServletRequest request) {
		...
		try {
			...
			partyId = StringUtils.isNullOrEmpty(partyId) ? getAnonymousId() : partyId;
...
//			content = URLDecoder.decode(content, "utf-8");
			...
			if (StringUtils.isNullOrEmpty(loginPartyId)) {
				...
				onlineChatVisitorMessageService.saveSend(getAnonymousId(), type, "send", content, null);
	...
	String getAnonymousId() {
		...
		if(anonymousId==null||anonymousId.trim().isEmpty()){
			throw new BusinessException("chat error");
		}
		return anonymousId;
	}
	/**
