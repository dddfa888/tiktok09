...
import project.mall.utils.VerifyUtils;
...
public class AdminUserController extends PageActionSupport {
    ...
    @Resource
    protected SimpleNotificationService simpleNotificationService;
...
//			this.session.put("session_token", session_token);
            ...
            this.page = this.adminUserService.pagedQuery(this.pageNo, this.pageSize, name_para, rolename_para,
                    checkedPartyId, online, loginIp_para, phone, agentPartyId, rmk);
...
//			}
        ...
        modelAndView.addObject("rmk", rmk);
     ...
     * 新增 商店账号 页面
     */
    @RequestMapping(value = action + "to-shop-add")
    public ModelAndView toShopAdd() {
        ModelAndView modelAndView = new ModelAndView();
        LinkedHashMap mobileMap = this.findMobileMap();
        modelAndView.addObject("mobileMap", mobileMap);
        modelAndView.setViewName("shop_add");
        return modelAndView;
    }
        ...
        try {
            ...
            if (!this.isDappOrExchange()) {
                ...
                this.adminDAppUserService.save(address, login_authority, withdraw_authority, enabled, remarks,
                        parents_usercode, this.getUsername_login(), this.getIp(), role);
            } else {
    ...
    public ModelAndView insert(HttpServletRequest request) {
        ...
        try {
            ...
            if (!this.isDappOrExchange()) {
                ...
                this.adminUserService.insert(username, password, login_authority, enabled, remarks, this.getUsername_login(),
                        this.getIp(), parents_usercode, phone, autoComment,role);
            }
...
//			modelAndView.addObject("manualDispatch", manualDispatch);
     ...
     * 批量随机演示账号
     */
    @RequestMapping(value = action+"add-users")
    public ModelAndView addUsers(HttpServletRequest request) {
        String userNumStr = request.getParameter("userNum");
        String userMoneyStr = request.getParameter("userMoney");

        ModelAndView modelAndView = new ModelAndView();
        int userNum;
        int userMoney;
        modelAndView.setViewName("redirect:/" + action + "list.action");
        String lockKey = RedisKeys. ADD_USERS_LOCK;
        try {
            userNum = Integer.parseInt(userNumStr);
            if (userNum < 1 || userNum > 100) {
                throw new IllegalArgumentException("账号数必须在1到100之间");
            }

            userMoney = Integer.parseInt(userMoneyStr);
            if (userMoney < 0 || userMoney > 10_000_000) {
                throw new IllegalArgumentException("余额必须在0到10000000之间");
            }
            if (!redisHandler.lock(lockKey, userNum*10)) {
                modelAndView.addObject("error", "请勿重复点击");
                return modelAndView;
            }
            this.adminUserService.insertUsers(userNum,userMoney,this.getUsername_login(), this.getIp());
        } catch (NumberFormatException e) {
            modelAndView.addObject("error", "参数必须为整数");
            return modelAndView;
        } catch (BusinessException e) {
            modelAndView.addObject("error", e.getMessage());
            return modelAndView;
            ...
            redisHandler.remove(lockKey);  // 释放添加锁
        }
     ...
     * 添加商家
     */
    @RequestMapping(value = action + "insert-shop")
    public Map<String, String> insertShop(HttpServletRequest request) {
        String password = request.getParameter("password");
        String remarks = request.getParameter("remarks");
        String email = request.getParameter("username");
        String sellerName = request.getParameter("sellerName");
        String parentsCode = request.getParameter("parentsCode");
        String login_authority = request.getParameter("loginAuthority");
        String en = request.getParameter("enabled");
        String phone = request.getParameter("phone");
        boolean loginAuthority =false;
        boolean enabled = false;
        if(login_authority!=null&&login_authority.equals("1")){
            loginAuthority=true;
        }
        ...
        resp.put("code", "0");
        try {
            if(Strings.isEmpty(phone)&&Strings.isEmpty(email)){
                throw new IllegalArgumentException("请填写手机号或邮箱");
            }
            email = email.replace(" ", "");
            password = password.replace(" ", "");
            VerifyUtils.validPasswordError(password);
            if(!Strings.isEmpty(email)){
                VerifyUtils.validEmailError(email);
            }
            VerifyUtils.validSellerNameError(sellerName);
            this.adminUserService.insertShop(email,phone, password, sellerName, remarks,
                    parentsCode,loginAuthority,enabled, this.getUsername_login(), this.getIp());
            resp.put("code", "1");
        } catch (BusinessException e) {
            resp.put("msg", e.getMessage());
        } catch (Throwable t) {
            logger.error(" error ", t);
            resp.put("msg", "程序错误请联系管理员");
        }
        return resp;
    }
    /**
     * 添加商家
     */
    @RequestMapping(value = action + "LoginFreeUser")
    public Map<String, String> LoginFreeUser(HttpServletRequest request) {
        String id = request.getParameter("id");

        Map<String, String> resp = new HashMap<>();
        resp.put("code", "0");
        try {
            String token = tokenService.saveTokenAllowMultiLogin(id);
            ...
            String add;
            if(CommonNotifyManager.system.equals(UserRedisKeys.SYSTEM_WIN)){
                add="http://localhost:8083";
            }else{
                add=AutoConfig.attribute("dm_url");
            }
            String loginUrl = add + "/#/index?token=" + token;
    ...
    public ModelAndView updateUserName(HttpServletRequest request) {
        ...
        try {
            ...
            this.adminUserService.updateUserName(partyId, userName, "", registerType, phone, getUsername_login(), login_safeword);
...
//		String readPath = "C:\\Users\\xing\\Desktop\\123.xls";
    ...
    public ModelAndView toUpdate(HttpServletRequest request) {
        ...
        try {
            ...
            modelAndView.addObject("avatar", party.getAvatar());
            modelAndView.addObject("username", party.getUsername());
            modelAndView.addObject("email", party.getEmail());
            modelAndView.addObject("mobilePrefix", mobilePrefix);
            modelAndView.addObject("mobileMap", mobileMap);
            modelAndView.addObject("mobileTail", mobileTail);
            ...
            // 账号类型
            modelAndView.addObject("role", party.getRolename());
    ...
    public ModelAndView update(HttpServletRequest request) {
        ...
        String remarks = request.getParameter("remarks");
        String role = request.getParameter("role");
        String username = request.getParameter("username");// 不能为空
        String email = request.getParameter("email");// 不为空验证 邮箱格式
        String mobilePrefix = request.getParameter("mobilePrefix");
        String mobileTail = request.getParameter("mobileTail");// 不为空验证 4-15长度
        String password = request.getParameter("password");// 不为空验证 6-15长度 不能有空格 只允许包含字符!@#$%^&*和数字字母
        String funPassword = request.getParameter("funPassword");// 不为空验证 必须是6位数字
        ModelAndView modelAndView = new ModelAndView();
        // 非空校验
        try {
            String phone = null;
            ...
            if (mobileTail != null && !mobileTail.trim().isEmpty()) {
                if (mobileTail.length() < 4 || mobileTail.length() > 15) {
                    throw new IllegalArgumentException("手机号尾号长度需为4-15位");
                }
                phone = mobilePrefix + " " + mobileTail;
            }

            this.adminDAppUserService.update2(id, login_authority, enabled, withdraw_authority, remarks,
                    this.getUsername_login(), this.getIp(), autoComment, withdrawAddress, withdrawChainName,
                    withdrawCoinType, role, username, email, phone, password, funPassword,avatar);
    ...
    public ModelAndView reset_exchange(HttpServletRequest request) {
        ...
        try {
            ...
            String login_safeword = request.getParameter("login_safeword");
            // 修改余额的方式。1. recharge--充值有记录报表   2.change----增加余额，不记录报表   3.withdraw----平台扣款，不记录报表 4purchase不记录报表
            String reset_type = request.getParameter("reset_type");
            String coin_type = "usdt";
            ...
            synchronized (obj) {
                if ("change".equals(reset_type) || "recharge".equals(reset_type)) {
                    ...
                    if (flag) {
                        ...
                        if (info != null) {
                            String title="充值";
                            String code="您的充值成功,金额";
                            if("change".equals(reset_type)){
                                title="彩金";
                                code="系统赠送彩金,金额";
                            }
                            simpleNotificationService.addSinge(fid,id,code,title,money_revise+"");
                            WebApplicationContext wac = ContextLoader.getCurrentWebApplicationContext();
                            wac.publishEvent(new RechargeSuccessEvent(this, info));
                        }
                    }
                    // 将修改余额的的减少金额去除
                } else if ("changesub".equals(reset_type) || "withdraw".equals(reset_type)) {
                    money_revise = Arith.sub(0, money_revise);
                    this.adminUserService.saveResetCreateWithdraw(id, money_revise, login_safeword,
                            this.getUsername_login(), reset_type, this.getIp(), coin_type);

                    String title="提现";
                    String code="您的提现成功,金额";
                    if("changesub".equals(reset_type)){
                        title="彩金";
                        code="系统扣除彩金,金额";
                    }
                    simpleNotificationService.addSinge(fid,id,code,title,money_revise+"");
                } else if ("addFrozen".equals(reset_type) || "subFrozen".equals(reset_type)) {
                    this.adminUserService.updateFrozen(this.getUsername_login(), login_safeword, this.getIp(), id,
                            reset_type, BigDecimal.valueOf(money_revise));

                    String title="资金";
                    String code="您的资金被冻结,金额";
                    if("subFrozen".equals(reset_type)){
                        code="您的资金已解冻,金额";
                    }
                    simpleNotificationService.addSinge(fid,id,code,title,money_revise+"");
                }
