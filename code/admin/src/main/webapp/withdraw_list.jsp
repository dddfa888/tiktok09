									...
									<div class="control-group">
										<div class="controls">
											...
											<select id="method" name="method"
												...
												<option value="wx"
														<c:if test="${method == 'wx'}">selected="true"</c:if>>微信</option>
												<option value="zfb"
														<c:if test="${method == 'zfb'}">selected="true"</c:if>>支付宝</option>
								...
								<c:choose>
									<c:when test="${item.method=='BANK'}">
										<td><a href="javascript:withdraw_blank_about('${item.amount}','${item.bankCardNo}','${item.bankUserName}','${item.bank}',
																					...
																					'${platformName}','${item.countryName}','${item.remark}')">查看信息</a></td>
									</c:when>
									<c:when test="${item.method=='WX'}">
										<td><a href="javascript:withdraw_wx_about('${item.amount}','${item.bankCardNo}','${item.qdcode}')">查看信息</a></td>
									</c:when>
									<c:when test="${item.method=='ZFB'}">
										<td><a href="javascript:withdraw_zfb_about('${item.amount}','${item.bankCardNo}','${item.bankUserName}')">查看信息</a></td>
									</c:when>
									<c:otherwise>
...
<%--															</div>--%>
	...
	<div>
		...
		<div class="enlargeContainer" id="enlargeContainer" onclick="clonsImg()">
			<!-- 关闭按钮，一个叉号图片 -->
			<span class="close2" data-dismiss="modal" aria-hidden="true">&times;</span>
			<img src="./image/close.png" class="close" id="close" onclick="clonsImg()" >
		</div>
	...
	<div class="modal fade" id="modal_wx_withdraw" tabindex="-1"
		 role="dialog" aria-labelledby="myModalLabel"
		 aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-header">
					...
					<h4 class="modal-title">微信提现</h4>
				</div>

				<div class="modal-body">
					<div class="">
						到账数量
						<input id="wx_amount" type="text"
							   name="wx_amount" class="form-control"
							   readonly="readonly" />
					</div>
					<div class="">
						微信绑定手机号
						<input id="wx_account" type="text"
							   name="wx_account" class="form-control"
							   readonly="readonly" />
					</div>
					<div class="modal-header">
						<h4 class="modal-title">微信收款码</h4>
					</div>

					<div class="modal-body">
						<div class="">
							<img width="200px" height="200px" id="wx_qdcode" name="wx_qdcode" src="" />
						</div>
					</div>
					<div class="modal-footer" style="margin-top: 0;">
						<button type="button" class="btn " onclick = "fz()">复制</button>
						<button type="button" class="btn " data-dismiss="modal">关闭</button>
					</div>
				</div>
				<!-- /.modal-content -->
	...
	<div class="modal fade" id="modal_withdraw" tabindex="-1"
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-header">
					<h4 class="modal-title" id="myModalLabel1">提现地址二维码</h4>
				</div>
...
<%--				</div>--%>
	...
	<div class="modal fade" id="modal_zfb_withdraw" tabindex="-1"
			 role="dialog" aria-labelledby="myModalLabel"
			 aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"
								aria-hidden="true">&times;</button>
						<h4 class="modal-title">支付宝提现</h4>
					</div>

					<div class="modal-body">
						<div class="">
							支付宝账号
							<input id="zfb_account" type="text"
								   name="zfb_account" class="form-control"
								   readonly="readonly" />
						</div>
						<div class="">
							姓名
							<input id="zfb_name" type="text"
								   name="zfb_name" class="form-control"
								   readonly="readonly" />
						</div>
						<div class="">
							到账数量
							<input id="zfb_amount" type="text"
								   name="zfb_amount" class="form-control"
								   readonly="readonly" />
						</div>
						<div class="modal-footer" style="margin-top: 0;">
							<button type="button" class="btn " onclick = "fz()">复制</button>
...
<script type="text/javascript">
	...
	function withdraw_blank_about(amount, bankCardNo, bankUserName, bank, routingNum, accountAddress, bankAddress, platformName, countryName,remark){
	...
	function 	withdraw_wx_about(amount, bankCardNo,qdcode){
		$("#wx_amount").val(amount);
		$("#wx_account").val(bankCardNo);
		document.getElementById('wx_qdcode').src=qdcode;
		$('#modal_wx_withdraw').modal("show");
	}
	function 	withdraw_zfb_about(amount, bankCardNo, bankUserName){
		$("#zfb_amount").val(amount);
		$("#zfb_account").val(bankCardNo);
		$("#zfb_name").val(bankUserName);
		$('#modal_zfb_withdraw').modal("show");
	}
...
<script type="text/javascript">
	...
	function openImg(id) {
		...
		console.log("点击的图片 src: " + toEnlargeImg.src);

		// 检查图片的路径是否为空
		if (!toEnlargeImg.src) {
			console.log("图片路径为空，无法展示大图");
			return;
		}
		...
		console.log("遮罩和容器已显示");

		// 创建并插入大图
		let img = new Image();
		img.src = toEnlargeImg.src;
		img.classList.add('enlargePreviewImg');
		console.log("大图创建完毕，src: " + img.src);
		...
		console.log("新图片已插入容器");
	}

	// 直接绑定图片的事件
	document.getElementById('withdraw_img').addEventListener('click', function() {
		openImg('withdraw_img');
	});
	document.getElementById('wx_qdcode').addEventListener('click', function() {
		console.log(123)
		openImg('wx_qdcode');
	});
	function clonsImg() {
