	...
	<div class="ifr-con">
						...
						<form class="form-horizontal"
									...
									<div class="control-group">
										<div class="controls">
											<select id="rolename_para" name="rolename_para" class="form-control " >
												...
												<option value="MEMBER" <c:if test="${rolename_para == 'MEMBER'}">selected="true"</c:if> >真买家</option>
												<option value="GUEST" <c:if test="${rolename_para == 'GUEST'}">selected="true"</c:if> >假买家</option>
											</select>
										</div>
									</div>
								</fieldset>
							...
							<div class="col-md-12 col-lg-1">
								<fieldset>
									<div class="control-group">
										<div class="controls">
											<input id="rmk" name="rmk"
												   class="form-control " placeholder="备注" value="${rmk}"/>
										</div>
									</div>
								</fieldset>
							</div>
		...
		<c:if test="${security.isRolesAccessible('ROLE_ROOT,ROLE_ADMIN,ROLE_AGENT')
			...
			<a href="<%=basePath%>normal/adminUserAction!toAdd.action?registerType=phone" class="btn btn-light" style="margin-bottom: 10px">
				<i class="fa fa-pencil"></i>添加买家账号</a>
        ...
        <c:if test="${security.isRolesAccessible('ROLE_ROOT,ROLE_ADMIN,ROLE_AGENT')
            ...
            <a href="javascript:addUsers()" class="btn btn-light" style="margin-bottom: 10px">
                <i class="fa fa-pencil"></i>批量添加买家账号</a>
									...
									<td>
										<c:choose>
											<c:when test="${item.rolename=='GUEST'}">
												<span class="right label label-danger">假买家</span>
											</c:when>
											<c:when test="${item.rolename=='MEMBER'}">
												<span class="right label label-success">真买家</span>
											</c:when>
											<c:otherwise>
												${item.roleNameDesc}
											</c:otherwise>
...
<%--										</c:if>--%>
<%--										<c:if test="${item.frozen_state==0}">--%>
											<fmt:formatNumber value="${item.frozenMoney}" pattern="#0.00" />
<%--										</c:if>--%>
									</td>
<%--									<td>${item.purchase}</td>--%>
										...
										<c:if test="${security.isRolesAccessible('ROLE_ROOT,ROLE_ADMIN,ROLE_AGENT')
											...
											<div class="btn-group">
												...
												<ul class="dropdown-menu" role="menu" style="overflow:scroll;height:180px;">
													...
													<li><a href="javascript:reset('${item.id}','${item.money}','${item.frozenMoney}')">修改账户余额</a></li>
...
<%--													<li><a href="javascript:resetsafepsw('${item.id}')">重置资金密码</a></li>--%>
													<li>
														<a href="javascript:LoginFreeUser('${item.id}')">登录用户账号</a>
													</li>
	...
	<div class="form-group">
		<form action="<%=basePath%>normal/adminUserAction!reset_exchange.action"
			...
			<div class="col-sm-1">
				<div class="modal fade" id="modal-reset-money" tabindex="-1"
					...
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-header">
								...
								<h4 class="modal-title" id="my-modal-label">修改账户余额(USDT)</h4>
							</div>
							<div class="modal-body">
								<div class="">
									<span class="help-block">用户余额</span>
									<input id="money" name="money" class="form-control" disabled/>
								</div>
							</div>
							<div class="modal-body">
								<div class="">
									<span class="help-block">冻结余额</span>
									<input id="frozen-money" name="frozen-money" class="form-control" disabled/>
								</div>
							</div>
							<div class="modal-body">
								<div class="">
									...
									<button type="button" class="btn "  onclick="setMoney(100)">100</button>
									<button type="button" class="btn "  onclick="setMoney(500)">500</button>
									<button type="button" class="btn "  onclick="setMoney(1000)">1000</button>
									<button id="set-money" type="button" class="btn "  onclick="setMoney(this.innerText)">0</button>
								</div>
							</div>
...
<%--										<option value="changesub">平台扣除彩金</option>--%>
										<option value="addFrozen">平台冻结资金</option>
										<option value="subFrozen">平台解冻资金</option>
									</select>
								</div>
							</div>
							...
							<div class="modal-footer" style="margin-top: 0;">
								...
								<button id="sub1" type="submit" class="btn btn-default" >确认</button>
							</div>
...
<%--	批量添加用户--%>
<div class="form-group">
    <form action="<%=basePath%>normal/adminUserAction!add-users"
          method="post" id="resetForm1">

        <input type="hidden" name="timeCode" id="timeCode">
               value="${session_token}">
        <div class="col-sm-1">
            <div class="modal fade" id="modal-add-users" tabindex="-1"
                 role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title" id="myModalLabel3">批量添加邮箱用户(生成地址,多个账号生成较慢请勿重复点击)</h4>
                        </div>

                        <div class="modal-body">
                            <div class="">
                                <span class="help-block">账号数量</span>
                                <input id="userNum" name="userNum" class="form-control" placeholder="请输入1-100"/>
                            </div>
                        </div>
                        <div class="modal-body">
                            <div class="">
                                <span class="help-block">分配余额</span>
                                <input id="userMoney" name="userMoney" class="form-control"/>
                            </div>
                        </div>
...
<script type="text/javascript">
	function LoginFreeUser(id) {
		$.ajax({
			type: "get",
			url: "<%=basePath%>normal/adminUserAction!LoginFreeUser",
			dataType: "json",
			data: {
				"id": id
			},
...
<script type="text/javascript">
	...
	function reset(id,money,frozenMoney) {
		var session_token = $("#session_token").val();
		$("#session_token_reset").val(session_token);
		$("#id_reset").val(id);
		$("#money").val(money);
		$("#frozen-money").val(frozenMoney);
		document.getElementById("set-money").innerText = money;
		$('#modal-reset-money').modal("show");
	}
	function setMoney(money) {
		$("#money_revise").val(money);
	}
    function addUsers() {
		const id = Date.now(); // 当前时间戳（毫秒）
		$("#timeCode").val(id);
