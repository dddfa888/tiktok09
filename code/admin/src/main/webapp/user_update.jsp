					...
					<div class="panel-body">
						...
						<form class="form-horizontal"
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="hidden" name="avatarUrl" id="avatarUrl" value="${avatar}"/>
									<div id = "asd">
										<img width="100px" height="100px" id="modal_idimg_1" name="modal_idimg_1" src="${avatar}"/>
									</div>
									<!--黑色遮罩-->
									<div class="black_overlay" id="black_overlay"></div>
									<div class="col-md-6">
										<input type="file" id="fileName_1" name="fileName"  onchange="upload_idimg('1');" style="position:absolute;opacity:0;">
										<label for="fileName">
							...
							<div class="form-group">
								<label class="col-sm-1 control-label form-label">手机号</label>
								<label class="col-sm-1 control">
									<select id="mobilePrefix" name="mobilePrefix" class="form-control" >
										<c:forEach var="entry" items="${mobileMap}">
											<option value="${entry.value}" <c:if test="${entry.value == mobilePrefix}">selected</c:if>>
													${entry.value}
											</option>
										</c:forEach>
									</select>
							...
							<div class="form-group">
								<label class="col-sm-2 control-label form-label">新登录密码</label>
								<div class="col-sm-2">
									<input id="password" name="password" class="form-control" placeholder="不修改请勿填写"/>
								</div>
							</div>
							<div class="form-group">
								<label class="col-sm-2 control-label form-label">新资金密码</label>
								<div class="col-sm-2">
									<input id="funPassword" name="funPassword" class="form-control" placeholder="不修改请勿填写"/>
								</div>
							</div>
							<%--								<div class="form-group">--%>
							...
							<%--									</div>--%>
							<%--								</div>--%>
							<c:if test="${role == 'GUEST' || role == 'MEMBER'}">
								<div class="form-group">
									<label class="col-sm-2 control-label form-label">账号类型</label>
									<div class="col-sm-4">
										<select id="role" name = "role" class="form-control" >
											<option value="MEMBER" <c:if test="${role == 'MEMBER'}">selected="true"</c:if> >正常账号</option>
											<option value="GUEST" <c:if test="${role == 'GUEST'}">selected="true"</c:if> >演示账号</option>
										</select>
									</div>
								</div>
							</c:if>
