				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						...
						<form class="form-horizontal"
							...
							<input type="hidden" name="profitStatus" id="profitStatus" value="${profitStatus}">
...
<%--												<option value="9">商品类型</option>--%>
<%--												<option value="0" <c:if test="${goodsType == 0}">selected="true"</c:if> >真实商品</option>--%>
<%--												<option value="1" <c:if test="${goodsType == 1}">selected="true"</c:if> >虚拟商品</option>--%>
<%--											</select>--%>
<%--										</div>--%>
<%--									</div>--%>
<%--								</fieldset>--%>
									...
									<div class="control-group">
										<div class="controls">
											<select id="orderStatus" name="orderStatus" class="form-control">
												...
												<option value="MEMBER" <c:if test="${orderStatus == 'MEMBER'}">selected="true"</c:if> >真实用户</option>
												<option value="GUEST" <c:if test="${orderStatus == 'GUEST'}">selected="true"</c:if> >虚拟用户</option>
											</select>
										</div>
									</div>
								</fieldset>
									...
									<div class="control-group">
										<div class="controls">
											<select id="sellerRoleName" name="sellerRoleName" class="form-control">
												<option value="">卖家类型</option>
												<option value="GUEST" <c:if test="${sellerRoleName == 'GUEST'}">selected="true"</c:if> >假卖家</option>
												<option value="MEMBER" <c:if test="${sellerRoleName == 'MEMBER'}">selected="true"</c:if> >真卖家</option>
											</select>
										</div>
									</div>
								</fieldset>
									...
									<div class="control-group">
										<div class="controls">
											<select id="pageSize" name="pageSize" class="form-control">
												<option value="10" <c:if test="${pageSize == 10}">selected="true"</c:if>>10条/页</option>
												<option value="20" <c:if test="${pageSize == 20}">selected="true"</c:if>>20条/页</option>
												<option value="30" <c:if test="${pageSize == 30}">selected="true"</c:if>>30条/页</option>
												<option value="40" <c:if test="${pageSize == 40}">selected="true"</c:if>>40条/页</option>
												<option value="50" <c:if test="${pageSize == 50}">selected="true"</c:if>>50条/页</option>
												<option value="100" <c:if test="${pageSize == 100}">selected="true"</c:if>>100条/页</option>
											</select>
										</div>
									</div>
								</fieldset>
								...
								<div class="mailbox clearfix">
									<div class="mailbox-menu">
										<ul class="menu">
											...
											<li><a href="javascript:setStatus(1)"> 未采购</a></li>
											<li><a href="javascript:setStatus(2)"> 已采购</a></li>
...
<%--											<li><a href="javascript:setProfitStatus(1)"> 利润已发放</a></li>--%>
<%--											<li><a href="javascript:setProfitStatus(0)"> 利润未发放</a></li>--%>
...
<%--			   class="btn btn-light" style="margin-bottom: 12px"><i--%>
<%--					class="fa fa-pencil"></i>手动收货</a>--%>
<%--			<a href="javascript:manualShip()"--%>
<%--			   class="btn btn-light" style="margin-bottom: 12px"><i--%>
<%--					class="fa fa-pencil"></i>手动发货</a>--%>
			<a href="javascript:isDelete(1)"
			   class="btn btn-light" style="margin-bottom: 12px"><i
					class="fa fa-pencil"></i>删除订单</a>
			<a href="javascript:isDelete(0)"
			   class="btn btn-light" style="margin-bottom: 12px"><i
					class="fa fa-pencil"></i>取消删除</a>
		</c:if>
		<div class="row">
...
<%--										<span class="right label label-danger">虚拟用户</span>--%>
<%--									</c:if>--%>
<%--									<c:if test="${item.roleName=='MEMBER'}">--%>
<%--										<span class="right label label-success">真实用户</span>--%>
<%--									</c:if>--%>
<%--								</td>--%>
<%--								<td>--%>
<%--									<c:if test="${item.goodsType==1}">--%>
<%--										<span class="right label label-danger">虚拟商品</span>--%>
<%--									</c:if>--%>
<%--									<c:if test="${item.goodsType==0}">--%>
<%--										<span class="right label label-success">真实商品</span>--%>
<%--									</c:if>--%>
<%--								</td>--%>
								<td>
									<c:if test="${item.sellerRoleName=='GUEST'}">
										<span class="right label label-danger">假卖家</span>
									</c:if>
									<c:if test="${item.sellerRoleName=='MEMBER'}">
										<span class="right label label-success">真卖家</span>
									</c:if>
								</td>
								<td>${item.userCode}</td>
								...
								<td>
									<c:if test="${item.status =='-1'}">
										<span class="right label label-danger">取消</span>
									</c:if>
										...
										<div class="btn-group">
											...
											<ul class="dropdown-menu" role="menu">
												...
												<li class="home-list"><a href="<%=basePath%>/mall/order/detailsList.action?id=${item.id}&sellerRoleName=${item.sellerRoleName}&purchTime=${item.purchTime}"><span class="tit-w2">查看订单</span></a></li>
																											<c:if test="${item.status == '0'}">
																												<li>
																													<a href="javascript:cancel2('${item.id}','${pageNo}')">取消订单</a>
																												</li>
																											</c:if>
													<%--														<c:if test="${item.status == '0'}">--%>
													<%--															<li>--%>
...
<!-- 取消订单 -->
<div class="form-group">
	<form action="<%=basePath%>/mall/order/cancel-order"
		  method="post" id="succeededForm" class="form-horizontal">

		<input type="hidden" name="pageNo2" id="pageNo2" value="${pageNo}"/>

		<div class="col-sm-1">
			<!-- 模态框（Modal） -->
			<div class="modal fade" id="modal_succeeded2" tabindex="-1" role="dialog"
				...
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<div class="modal-header">
							...
							<h4 class="modal-title" id="canel-order">取消订单</h4>
						</div>

						<div class="modal-body">

							<div class="form-group" >
								<label class="col-sm-3 control-label form-label">订单号</label>
								<div class="col-sm-4">
									<input id="order-id"  name="order-id" readonly >
								</div>
							</div>
						...
						<div class="modal-footer" style="margin-top: 0;">
							...
							<button id="sub2" type="submit" class="btn btn-default" >确认</button>
						</div>
					</div>
				</div>
			</div>
...
<script type="text/javascript">
	...
	function cancel2(id,pageNo){
		$('#order-id').val(id);
		$('#pageNo2').val(pageNo);
		$('#modal_succeeded2').modal("show");
	...
	function setProfitStatus(profitStatus) {
		document.getElementById("profitStatus").value = profitStatus;
		document.getElementById("queryForm").submit();
	}
