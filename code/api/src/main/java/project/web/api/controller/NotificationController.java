...
//        Seller pl = sellerService.getSeller(partyId);
    ...
    @GetMapping(action + "tip-list")
    public Object getListByCode() {
        ResultObject resultObject = new ResultObject();
        String partyId = this.getLoginPartyId();
        List<SimpleNotificationVo> vo = simpleNotificationService.updateAndGet(partyId);
        resultObject.setMsg("ok");
        resultObject.setData(vo);
        return resultObject;
    }
    @GetMapping(action + "tip-count")
    public Object getTipUnreadCount() {
        ResultObject resultObject = new ResultObject();
        String partyId = this.getLoginPartyId();
        Long count = simpleNotificationService.getCount(partyId);
        resultObject.setMsg("ok");
