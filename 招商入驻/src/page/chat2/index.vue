    ...
    <div class="content" style="flex: 1;overflow-y: scroll;" ref="content">
      <div class="flex flex-col px-6 box-border" style="margin-bottom: 24px;" ref="msg">
        ...
        <ul class="flex flex-col pt-20">
          ...
          :class="!item.delete_status ? '' : 'flex flex-col mt-20'">

            <!--            {{list}}111-->
            <p v-if="item.showTime && !item.delete_status" class="font-16 text-center py-20 text-grey">
              <!--              {{ item.createtime && item.createtime.split(' ')[0] }}-->
              {{ item.createtime && item.createtime }}
            </p>
            <div v-if="!item.delete_status" :class="item.send_receive === 'send' ? 'justify-end': ''" class="flex">
